...
class ViewModel extends think\Model
{
    ...
    public function setting()
    {
        ...
        if (0 < count($yaml) && !empty($yaml["config-parent-theme"])) {
    ...
    public function nList($param, $cate_id)
    {
        ...
        if ($param["data"] == "ListCate") {
            ...
            $page = 0 < intval($param["html3"]) ? intval($param["html3"]) : 1;
        } else if ($param["data"] == "Search") {
            $page = 0 < intval($param["page"]) ? intval($param["page"]) : 1;
        } else {
            $page = 0 < intval($param["html2"]) ? intval($param["html2"]) : 1;
        }
        $where = [["hidden", "=", 0]];
        ...
        if (1 < $returndata["total_page"]) {
            $num = 3;
            $returndata["previous"] = $page == 1 ? 0 : $page - 1;
            $returndata["next"] = $returndata["total_page"] <= $page ? 0 : $page + 1;
            $returndata["start"] = $page;
            $returndata["num"] = $returndata["total_page"] < $page + $num ? $returndata["total_page"] : $page + $num;
            if ($returndata["num"] - $returndata["start"] < $num) {
                $returndata["start"] = 0 < $returndata["num"] - $num ? $returndata["num"] - $num : 1;
            }
    ...
    {
        if (0 < count($tagDataVal) && is_array($tagDataVal)) {
