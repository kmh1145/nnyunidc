...
class LoginController extends cmf\controller\HomeBaseController
{
    ...
    public function phonePassLogin()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $validate = new \think\Validate(["phone" => "require|length:4,11", "password" => "require|min:6|max:255"]);
            $validate->message(["phone.require" => "手机号不能为空", "phone.length" => "手机长度为4-11位", "password.require" => "密码必填", "password.min" => "密码至少6位", "password.max" => "密码最多255位"]);
            if (!$validate->check($data)) {
    ...
    public function emailLogin()
    {
        if ($this->request->isPost()) {
            $data = $this->request->param();
            $validate = new \think\Validate(["email" => "require", "password" => "require|min:6|max:255"]);
            $validate->message(["email.require" => "邮箱不能为空", "password.require" => "密码不能为空", "password.min" => "密码至少6位", "password.max" => "密码最多255位"]);
            if (!$validate->check($data)) {
                return jsons(["status" => 400, "msg" => $validate->getError()]);
            }
            ...
            if (0 < strpos($email, "@")) {
                ...
                $validate->message(["email.require" => "邮箱不能为空", "password.require" => "密码必填", "password.min" => "密码至少6位", "password.max" => "密码最多255位"]);
    ...
    public function verify()
    {
        ...
        if ($data["is_captcha"] == 1) {
            ...
            if ($data[$param["name"]] != 1) {
                return jsons(["status" => 400, "msg" => "未开启验证码1"]);
            }
        ...
        return jsons(["status" => 400, "msg" => "未开启验证码2"]);
    }
}
