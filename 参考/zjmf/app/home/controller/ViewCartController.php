...
class ViewCartController extends ViewBaseController
{
    ...
    public function cart(\think\Request $request)
    {
        ...
        if ($action == "configureproduct") {
            if (getZjmfApiIdByProductId($param["pid"])) {
                $type = getZjmfApiProductTypeByProductId($param["pid"]);
                if ($type == "dcimcloud") {
                    return $this->view("configureproduct-v10-cloud", $this->confProductFun($request, $cart, $param));
                }
                if ($type == "dcim") {
                    return $this->view("configureproduct-v10-dcim", $this->confProductFun($request, $cart, $param));
                }
                return $this->view("configureproduct-v10-common", $this->confProductFun($request, $cart, $param));
            }
        ...
        } else if ($param["pid"] && ($param["carttheme"] == "area" || cookie("cart_theme") == "area")) {
    ...
    protected function productFun($request, $cart, $param, $products)
    {
        ...
        if ($products) {
            $param["pid"] = $param["pid"] ?: $products[0]["id"] ?? NULL;
        ...
        if ($_second_k != 0) {
            $param["gid"] = $_second_k;
            $request->gid = $param["gid"];
        }
        if ($_first_k != 0) {
            $param["fid"] = $_first_k;
            $request->fid = $param["fid"];
        }
    ...
    protected function viewCartFun($request, $cart, $param)
    {
        ...
        if ($request->isPost() && $param["statuscart"] == "change") {
            $removeProduct = $cart->modifyProductQty($request);
            header("location:" . $this->ViewModel->domain . "/cart?action=viewcart" . $url_param);
            exit;
        }
        if ($request->isPost() && $param["statuscart"] == "removepromo") {
        ...
        } else if ($request->isPost() && $param["statuscart"] == "promo") {
        ...
        } else if ($request->isPost() && $param["statuscart"] == "checkout") {
            if (!$param["terms"]) {
                $data["ErrorMsg"] = "请勾选服务条款";
            } else {
                if (!request()->uid) {
                    if ($param["register_or_login"] == "login" && $request->isPost()) {
                        if ($param["phone_code"]) {
                ...
                if ($result["status"] == 200 || request()->uid) {
                    ...
                    if ($settle["status"] == 200) {
                        ...
                        } else if ($param["paymt"] === "credit_limit") {
                            $request->invoiceid = $settle["data"]["invoiceid"];
            ...
            foreach ($setConfig["product"]["cycle"] as $cycleKey => $cycle) {
                if ($cycle["billingcycle"] == "ontrial" || $cycle["billingcycle"] == "free" || $cycle["billingcycle"] == "onetime") {
                    ...
                    if (empty($billing_cycle_base)) {
                        ...
                        } else if ($cycle["billingcycle"] == "day") {
                            $cycle_base_pricing = $getTotal2["products"]["total"] / $cycle["day_cycle"];
                        }
                    } else if (!empty($billing_cycle_base)) {
