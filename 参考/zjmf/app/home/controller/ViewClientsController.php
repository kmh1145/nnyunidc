...
class ViewClientsController extends ViewBaseController
{
    ...
    public function cloudVnc(\think\Request $request)
    {
        $param = $request->param();
        $cache = \think\facade\Cache::get("mf_cloud_vnc_" . $param["id"]);
        return $this->view("v10/view/vnc_page", $cache);
    }
    public function dcimVnc(\think\Request $request)
    {
        $param = $request->param();
        $cache = \think\facade\Cache::get("mf_dcim_vnc_" . $param["id"]);
        return $this->view("v10/viewdcim/vnc_page", $cache);
    }
    public function login(\think\Request $request)
    {
        ...
        if ($LoginRegisterIndex["data"]["allow_second_verify"] == 1 && in_array("login", $LoginRegisterIndex["data"]["second_verify_action_home"])) {
    ...
    public function loginAccessToken(\think\Request $request)
    {
        ...
        if ($request->isPost()) {
            ...
            if ($result["status"] == 200) {
                ...
                if ($res_check === false) {
                    $redirect_url .= "?access_token=" . $token . "&jwt=" . $result["jwt"];
                } else {
                    $redirect_url .= "&access_token=" . $token . "&jwt=" . $result["jwt"];
                }
    ...
    public function clientarea(\think\Request $request)
    {
        ...
        $limit = 1 <= $param["limit"] ? intval($param["limit"]) : 5;
    ...
    public function verified(\think\Request $request)
    {
        ...
        if ($request->isPost() && empty($error)) {
            ...
            } else if ($action == "enterprises" && $step == "info") {
                $uploadImage = $this->verifiedUploadImage();
                if ($uploadImage["status"] !== 200) {
                    $data["ErrorMsg"] = $uploadImage["msg"];
                } else {
        ...
        if ($plugin != "artificial") {
            ...
            if (0 < floatval($_config["amount"]) && $pay) {
        ...
        if (isset($certifi_person["id"]) && $certifi_person["status"] == 1 && isset($certifi_company["status"]) && $certifi_company["status"] == 2) {
    ...
    public function security(\think\Request $request)
    {
        ...
        foreach ($arr as $val) {
            $val == 1 or $val == 1 && $number++;
        }
        if (33 < intval($number / $count * 100)) {
            $t = get_title_lang("security_moderate");
        }
        if (66 < intval($number / $count * 100)) {
    ...
    public function service(\think\Request $request)
    {
        ...
        if (!file_exists(CMF_ROOT . "public/themes/clientarea/" . configuration("clientarea_default_themes") . "/" . $nav_info["templatePage"] . ".tpl") && !file_exists(CMF_ROOT . "public/themes/clientarea/default/" . $nav_info["templatePage"] . ".tpl")) {
        ...
        if ($nav_info["templatePage"] == "service_ssl" && $domain_status_empty) {
    ...
    public function servicedetail(\think\Request $request)
    {
        ...
        if ($request->isPost()) {
            if ($action == "cancel") {
                ...
                if ($action == "upgrade_use_promo_code") {
                    $Upgrade->request->hid = $request->id;
                ...
                if ($action == "upgrade_remove_promo_code") {
                    $Upgrade->request->hid = $request->id;
                ...
                if ($action == "renew") {
                    ...
                    if ($action == "upgrade_config_use_promo_code") {
                        $Upgrade->request->hid = $request->id;
                    ...
                    if ($action == "upgrade_config_remove_promo_code") {
                        $Upgrade->request->hid = $request->id;
        ...
        } else if ($getHeader["data"]["host_cancel"]["type"] == "Endofbilling" && configuration("cron_host_terminate") == 1) {
        ...
        if (getZjmfApiIdByHostId($request->id)) {
            $productid = \think\Db::name("host")->where("id", $request->id)->value("productid");
            $type = getZjmfApiProductTypeByProductId($productid);
            if ($type == "dcimcloud") {
                return $this->view("servicedetail-v10-cloud", $data);
            }
            if ($type == "dcim") {
                return $this->view("servicedetail-v10-dcim", $data);
            }
            return $this->view("servicedetail-v10-common", $data);
        }
    ...
    public function pay(\think\Request $request)
    {
        ...
        if ($action == "recharge") {
            ...
            if ($invoice["type"] != "recharge") {
                ...
                if (0 < $user_credit) {
                    if (0 < $user_credit && $invoice["subtotal"] == $invoice["total"] && $param["use_credit"] == 1) {
                    ...
                    } else if ($param["use_credit"] == 0 && isset($param["use_credit"])) {
            ...
            if ($invoice["type"] == "recharge") {
                ...
                if ($subtotal == $total) {
                    if ($credit <= 0) {
                        ...
                        } else if (!is_null($param["use_credit"])) {
                            $apply_credit = $Pay->applyCredit($request);
