...
class CustomfieldsValidate extends think\Validate
{
    ...
    protected function checkCustomfieldname($array)
    {
        foreach ($array as $value) {
            if (!empty($value)) {
                $status = preg_match("/^[\\x{4e00}-\\x{9fa5}a-zA-Z0-9\\_\\-]+\$/u", (string) $value);
                if (!$status) {
    ...
    protected function checkCustomfieldtype($array)
    {
        foreach ($array as $item) {
            if (!empty($item) && !in_array($item, ["text", "link", "password", "dropdown", "tickbox", "textarea"])) {
                return "自定义字段类型错误";
            }
        }
        return true;
    ...
    protected function checkCustomfieldoptions($array)
    {
        foreach ($array as $value) {
            if (!empty($value)) {
                $arr = explode(",", $value);
                foreach ($arr as $v) {
                    $status = preg_match("/^[\\x{4e00}-\\x{9fa5}a-zA-Z0-9]+\$/u", (string) $v);
                    if (!$status) {
    ...
    {
        foreach ($array as $item) {
            if (!empty($item) && !in_array($item, ["1", "on", "yes"])) {
