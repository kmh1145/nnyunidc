...
class DownloadsModel extends think\Model
{
    ...
    {
        $downloads = \think\Db::name("downloads")->field("id,category,type,title,description,downloads")->where("hidden", 0)->order("downloads DESC")->order("sort ASC")->limit(5)->where(function ($query) {
            static $uid = NULL;
            if (empty($uid)) {
    ...
    {
        $downloads = \think\Db::name("downloads")->field("id,category,type,title,description,downloads")->order("downloads DESC")->order("id DESC")->where(function ($query) {
            static $uid = NULL;
        })->select()->toArray();
        foreach ($downloads as $key => $val) {
            $downloads[$key]["down_link"] = "download/product_file?id=" . $val["id"];
    ...
    {
        $downloads = \think\Db::name("downloads")->field("id,type,title,description,downloads")->where("title LIKE '%" . $search . "%'")->whereOr("description LIKE '%" . $search . "%'")->where(function ($query) {
            static $uid = NULL;
            if (empty($uid)) {
                $query->where("clientsonly", 0);
            }
