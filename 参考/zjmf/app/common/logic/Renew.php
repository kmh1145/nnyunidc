...
class Renew
{
    ...
    public function batchRenew($hids, $billingcycles)
    {
        ...
        foreach ($billingcycles as $k => $billingcycle) {
            if (getZjmfApiIdByHostId($k)) {
            } else if (!in_array($billingcycle, array_keys(config("billing_cycle")))) {
                return ["status" => 400, "msg" => lang("CYCLE_ERROR")];
            }
        }
        ...
        try {
            foreach ($hids as $hid) {
                if ($apiId = getZjmfApiIdByHostId($hid)) {
                    $host_data = \think\Db::name("host")->field("h.id,h.uid,h.orderid,h.productid,h.domain,h.amount,h.promoid,h.payment,h.billingcycle,h.nextduedate,h.nextinvoicedate,h.domainstatus,h.dedicatedip,p.name as productname,p.pay_method,p.pay_type,i.status,h.flag,h.dcimid")->alias("h")->leftJoin("products p", "p.id=h.productid")->leftJoin("orders o", "h.orderid = o.id")->leftJoin("invoices i", "o.invoiceid = i.id")->where("h.id", $hid)->where("i.delete_time", 0)->where("o.delete_time", 0)->find();
                    $result = zjmfCurl($apiId, "/console/v1/host/" . $host_data["dcimid"] . "/renew", [], 30, "GET");
                    if ($result["status"] == 200) {
                        $product = \think\Db::name("products")->find($host_data["productid"]);
                        $flag = getSaleProductUser($host_data["productid"], $host_data["uid"]);
                        $billingcycle = config("coupon_cycle_promo")[$host_data["billingcycle"]] ?? $host_data["billingcycle"];
                        foreach ($result["data"]["host"] as &$item) {
                            if (strpos($billingcycle, $item["billing_cycle"]) !== false || strpos($item["billing_cycle"], $billingcycle) !== false) {
                                $item["price"] = bcmul($host_data["amount"], 1, 2);
                            } else {
                                $item["price"] = bcmul($item["price"] + ($item["discount"] ?? 0), $product["upstream_price_value"] / 100, 2);
                                $userdiscount = NULL;
                                if ($flag) {
                                    if ($flag["type"] == 1) {
                                        $userdiscount = bcmul($item["price"], 1 - $flag["bates"] / 100, 2);
                                        $item["price"] = bcmul($item["price"], $flag["bates"] / 100, 2);
                                    } else if ($flag["type"] == 2) {
                                        $userdiscount = $flag["bates"];
                                        $item["price"] = 0 < bcsub($item["price"], $flag["bates"], 2) ? bcsub($item["price"], $flag["bates"], 2) : 0;
                                    }
                                }
                            }
                            if (isset($billingcycles[$host_data["id"]])) {
                                if ($billingcycles[$host_data["id"]] == $item["billing_cycle"]) {
                                    $v10Amount = $item["price"];
                                    $duration = $item["duration"];
                                    if (isset($userdiscount)) {
                                        $userdiscountCurrent = $userdiscount;
                                    }
                                }
                        ...
                        if (isset($duration) && isset($v10Amount) && 0 <= $v10Amount) {
                            if (isset($userdiscountCurrent) && 0 < $userdiscountCurrent) {
                                $invoice_items1[] = ["uid" => $host_data["uid"], "type" => "discount", "rel_id" => $hid, "description" => "客戶折扣", "amount" => "-" . $userdiscountCurrent];
                            }
                            $start_time = date("Y/m/d H", $host_data["nextduedate"]);
                            $end_time = date("Y/m/d H", $host_data["nextduedate"] + $duration);
                            $invoice_items_data[] = ["uid" => $host_data["uid"], "type" => "renew", "rel_id" => $hid, "description" => "续费账单 - " . $host_data["productname"] . "(" . $host_data["domain"] . "),IP(" . $host_data["dedicatedip"] . "),购买时长：" . $billingcycles[$hid] . "(" . $start_time . "-" . $end_time . ")", "amount" => bcadd($v10Amount, $userdiscountCurrent ?? 0, 2)];
                            $renew_items_data[] = ["uid" => $host_data["uid"], "type" => "normal", "new_cycle" => $billingcycles[$hid], "new_recurring_amount" => bcadd($v10Amount, $userdiscountCurrent ?? 0, 2), "status" => "Pending", "paid" => "N", "create_time" => time(), "expire_time" => time(), "hid" => $hid];
                            $total = bcadd($total, $v10Amount, 2);
                        }
                    }
                } else {
                    ...
                    if ($host_data["api_type"] == "resource") {
                        ...
                        $userdiscount = 0;
                    } else if (!empty($billingcycle) && $billingcycle != $host_data["billingcycle"]) {
                        if ($amount_custom) {
                            $amount = floatval($amount_custom[$hid]);
                        } else if ((new \app\common\model\ProductModel())->checkProductPrice($pid, $billingcycle, $currency_id)) {
                            $amount1 = $this->calculatedPrice($hid, $billingcycle, 1, $host_data["flag"]);
                            $amount = $amount1["price_cycle"];
                    ...
                    if (0 <= $amount && !in_array($billingcycle, ["free", "ontrial"]) && $pay_method == "prepayment" && in_array($domainstatus, ["Active", "Suspended"])) {
                        ...
                        if (!empty($has) && $has["status"] == "Unpaid") {
                            ...
                            $invoice_credit = 0 < $invoice_credit ? $invoice_credit : 0;
            ...
            $insert_invoices = ["uid" => $uid, "create_time" => $create_time ?? time(), "due_time" => $expire_time ?? time(), "paid_time" => 0, "subtotal" => $total, "credit" => 0, "total" => $total, "status" => "Unpaid", "type" => "renew", "payment" => $payment, "url" => $url ?? ""];
            if (0 < $total) {
    ...
    public function renew($hid, $billingcycle, $duration = 0)
    {
        if (empty($hid)) {
            return ["status" => 400, "msg" => lang("ID_ERROR")];
        }
        $v10Api = getZjmfApiIdByHostId($hid);
        if (!$v10Api && (empty($billingcycle) || !in_array($billingcycle, array_keys(config("billing_cycle"))))) {
            return ["status" => 400, "msg" => lang("CYCLE_ERROR")];
        }
        $host_data = \think\Db::name("host")->field("h.id,h.uid,h.orderid,h.productid,h.domain,h.amount,h.promoid,h.payment,h.billingcycle,\r\n            h.nextduedate,h.nextinvoicedate,h.domainstatus,h.dedicatedip,p.name as productname,p.pay_method,\r\n            p.pay_type,i.status,h.flag,p.api_type,p.upstream_price_value,h.dcimid")->alias("h")->leftJoin("products p", "p.id=h.productid")->leftJoin("orders o", "h.orderid = o.id")->leftJoin("invoices i", "o.invoiceid = i.id")->where("h.id", $hid)->where("i.delete_time", 0)->where("o.delete_time", 0)->find();
        $promoid = $host_data["promoid"];
        if (!empty($host_data)) {
            ...
            $host_data = \think\Db::name("host")->field("h.id,h.uid,h.orderid,h.productid,h.domain,h.amount,h.promoid,h.payment,h.billingcycle,\r\n                h.nextduedate,h.nextinvoicedate,h.domainstatus,h.dedicatedip,p.name as productname,p.pay_method,\r\n                p.pay_type,h.flag,p.api_type,p.upstream_price_value,h.dcimid")->alias("h")->leftJoin("products p", "p.id=h.productid")->where("h.id", $hid)->find();
            $uid = $host_data["uid"];
            $currency_id = priorityCurrency($uid);
            $pid = $host_data["productid"];
        }
        if ($v10Api) {
            $amount = 0;
            $userdiscount = 0;
            $result = zjmfCurl($v10Api, "/console/v1/host/" . $host_data["dcimid"] . "/renew", [], 30, "GET");
            if ($result["status"] == 200) {
                $product = \think\Db::name("products")->find($host_data["productid"]);
                $flag = getSaleProductUser($host_data["productid"], $host_data["uid"]);
                $billingcycleCurrent = config("coupon_cycle_promo")[$host_data["billingcycle"]] ?? $host_data["billingcycle"];
                if (strpos($billingcycle, $billingcycleCurrent) !== false || strpos($billingcycleCurrent, $billingcycle) !== false) {
                    $amount = $host_data["amount"];
                } else {
                    foreach ($result["data"]["host"] as $item) {
                        if ($item["billing_cycle"] == $billingcycle) {
                            $amount = bcmul($item["price"] + $item["discount"], $product["upstream_price_value"], 2) / 100;
                            if ($flag) {
                                if ($flag["type"] == 1) {
                                    $userdiscount = bcmul($amount, (100 - $flag["bates"]) / 100, 2);
                                    $amount = bcmul($amount, $flag["bates"] / 100, 2);
                                } else if ($flag["type"] == 2) {
                                    $userdiscount = $flag["bates"];
                                    $amount = 0 < bcsub($amount, $flag["bates"], 2) ? bcsub($amount, $flag["bates"], 2) : 0;
                                }
                            }
                        }
                    }
        ...
        if (0 <= $amount && !in_array($billingcycle, ["free", "ontrial"]) && $pay_method == "prepayment" && in_array($domainstatus, ["Active", "Suspended"])) {
            if ($v10Api) {
                $start_time = date("Y/m/d H", $host_data["nextduedate"]);
                $end_time = date("Y/m/d H", $host_data["nextduedate"] + $duration);
                $invoice_items = ["uid" => $uid, "type" => "renew", "rel_id" => $hid, "description" => "续费账单 - " . $host_data["productname"] . "(" . $host_data["domain"] . "),IP(" . $host_data["dedicatedip"] . "),购买时长：" . $billingcycle . "(" . $start_time . "-" . $end_time . ")", "amount" => bcadd($amount, $userdiscount, 2)];
            } else {
                $pay_type = json_decode($host_data["pay_type"], true);
                $start_time = date("Y/m/d H", $host_data["nextduedate"]);
                $end_time = date("Y/m/d H", getNextTime($billingcycle, $pay_type["pay_" . $billingcycle . "_cycle"], $host_data["nextduedate"], $pay_type["pay_ontrial_cycle_type"] ?: "day"));
            ...
            $renew_items = ["uid" => $uid, "type" => "normal", "new_cycle" => $billingcycle, "new_recurring_amount" => $amount, "status" => "Pending", "paid" => "N", "create_time" => $create_time, "expire_time" => $expire_time, "duration" => $duration];
            ...
            if (0 < $amount) {
                ...
                $this->renewHandleIn($billingcycle, $hid, $duration);
    ...
    public function calculatedPrice($hostid, $compute_cycle = "", $type = 0, $flags = 1)
    {
        ...
        if ($compute_cycle) {
            ...
            if (0 < $product_pricing[$itself_price]) {
                ...
                if (!empty($configgroup)) {
                    ...
                    foreach ($options as $okey => $option) {
                        ...
                        if (empty($user_config_data)) {
                            ...
                            } else if ($flag && $flag["type"] == 1 && $option["is_discount"] == 1) {
                            ...
                            } else if (judgeQuantity($option_type)) {
                                if (judgeQuantityStage($option_type)) {
                                    $sum = quantityStagePrice($option["id"], $currency, $qty, $billingcycle);
                                    $price_cycle += $sum[0];
                                } else {
            ...
            return 0 <= $price_cycle ? $price_cycle : 0;
        } else {
            return 0;
        }
    }
    protected function renewHandleIn($billingcycle, $hid, $duration = 0)
    {
        $Host = new Host();
        ...
        if ($v10Api = getZjmfApiIdByHostId($hid)) {
            $next_time = $res["nextduedate"] + $duration;
        } else if (!empty($res)) {
    ...
    public function renewHandle($id)
    {
        ...
        if ($v10Api = getZjmfApiIdByHostId($res["hostid"] ?? 0)) {
            $next_time = time() + $renew_cycle["duration"];
        } else if (!empty($res)) {
            $pay_type = json_decode($res["pay_type"], true);
            if ($billingcycle == "onetime" || $billingcycle == "free") {
                $next_time = 0;
        ...
        } else if (configuration("shd_allow_email_send")) {
            $email = new Email();
            $email->sendEmailBase($res["hostid"], "Service_Unsuspension_Notification", "product", true);
        }
        $message_template_type = array_column(config("message_template_type"), "id", "name");
