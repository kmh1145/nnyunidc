...
class Dcim
{
    ...
    public function on($id, $log = true)
    {
        ...
        if ($product["api_type"] == "zjmf_api") {
        ...
        } else if ($product["api_type"] == "whmcs") {
            ...
            } else if ($product["config_option1"] == "bms") {
    ...
    public function traffic($id, $params = [])
    {
        ...
        if ($product["api_type"] == "zjmf_api") {
            ...
            } else if ($res["nosupport"] == "nosupport") {
                $result["status"] = 400;
                $result["msg"] = "不支持流量图";
        ...
        if ($product["api_type"] == "zjmf_api") {
            ...
            if ($product["config_option1"] == "rent") {
                ...
                if ($res["status"] == "success") {
                    ...
                    if ($is_common) {
                        $link_url = "";
                        if (isset($res["vnc_link_url"]) && !empty($res["vnc_link_url"])) {
                        ...
                        if (isset($res["vnc_link_url"]) && !empty($res["vnc_link_url"])) {
                            if (request()->scheme() == "https") {
                                $url = "wss://" . $res["vnc_link_url"];
                            } else {
                                $url = "ws://" . $res["vnc_link_url"];
                            }
                        } else {
                            if (strpos($this->url, "https://") !== false) {
    ...
    public function reinstall($id, $params = [])
    {
        ...
        if ($product["api_type"] == "zjmf_api") {
            ...
            } else if ($result["status"] == 400 && !$result["confirm"] && !isset($result["price"])) {
                if ($this->is_admin) {
                    $result["msg"] = $this->log_prefix_error . $result["msg"];
            ...
            if ($product["config_option1"] == "rent") {
                ...
                if ($res["status"] == "success") {
                    ...
                    $result["msg"] = "重装发起成功";
                    $description = sprintf("重装系统为" . $os_name . "发起成功 - Host ID:%d", $id);
                    if (!$this->is_admin && 0 < $product["reinstall_times"]) {
                    ...
                    if (!empty($res["data"])) {
                        ...
                        if (isset($res["last_result"]) && $product["show_last_act_message"] == 1 && ($res["last_result"]["type"] == 1 || $res["last_result"]["type"] == 2)) {
                            if ($res["last_result"]["task_type"] == 3) {
                                $info = "获取硬件信息";
                            } else if ($res["last_result"]["task_type"] == 2) {
                                $info = "重置密码";
                            } else if ($res["last_result"]["task_type"] == 1) {
                                $info = "救援系统";
                            } else {
                                $info = "重装系统";
                            }
            ...
            if ($product["config_option1"] == "rent") {
                ...
                if ($res["code"] == 200) {
                    ...
                    if ($res["status"] == "on") {
                        ...
                        $result["data"]["status"] = "suspend";
                        $result["data"]["des"] = "暂停";
                    } else if ($res["status"] == "wait_reboot") {
                        $result["data"]["status"] = "waiting";
                        $result["data"]["des"] = "等待重启";
    ...
    public function createAccount($hostid)
    {
        ...
        if ($params["type"] == "dcim") {
            ...
            if ($params["config_option1"] == "cabinet") {
            ...
            } else if ($params["config_option1"] == "rent") {
                ...
                if ($dcim_server["auth"]["enable_ip_custom"] == "on") {
                    $postfields["ip_customid"] = (int) $dcim_server["ip_customid"];
                }
                ...
                } else if (strpos($bw, ",") !== false) {
                    $bw_arr = explode(",", $bw);
                    list($postfields["in_bw"], $postfields["out_bw"]) = $bw_arr;
                } else if (is_numeric($bw) && 0 <= $bw) {
                    $postfields["in_bw"] = $bw;
                ...
                if ($res["status"] == "success") {
                    ...
                    if (is_numeric($data["decimal"]) && 0 < $data["decimal"]) {
                        $update["bwlimit"] = (int) ($update["bwlimit"] / $data["decimal"]);
                    }
                    $update["bwusage"] = 0;
            ...
            } else if ($params["config_option1"] == "bms") {
                $post_data = [];
                ...
                $post_data["group_id"] = (int) $params["configoptions"]["group_id"];
                if (isset($params["configoptions"]["node"])) {
                    $post_data["lc_id"] = (int) $params["configoptions"]["node"];
                }
                $post_data["os_id"] = (int) $params["configoptions"]["os"];
                ...
                $post_data["ip_num"] = (int) $params["configoptions"]["ip_num"];
                $post_data["ip_group_id"] = $params["configoptions"]["ip_group"];
                if (0 < $params["configoptions"]["data_disk_size"]) {
                    $post_data["data_size"] = (int) $params["configoptions"]["data_disk_size"];
                }
                if (0 < $params["configoptions"]["ceph_id"]) {
                    $post_data["ceph_id"] = (int) $params["configoptions"]["ceph_id"];
                }
    ...
    public function assignServer($id, $dcimid)
    {
        ...
        if ($product["config_option1"] == "rent") {
            ...
            if ($dcim_server["auth"]["enable_ip_custom"] == "on") {
                $post_data["ip_customid"] = (int) $dcim_server["ip_customid"];
            }
    ...
    public function resetFlow($hostid, $dcimid, $nextduedate = 0)
    {
        ...
        if ($res["status"] == "success") {
            ...
            if (isset($res["limit_flow"])) {
                $update["bwlimit"] = (int) $res["limit_flow"];
            }
            $update["bwusage"] = 0;
            $update["lastupdate"] = time();
            \think\Db::name("host")->where("id", $hostid)->update($update);
    ...
    public function moduleClientArea($id)
    {
        ...
        $name = "";
        if (0 <= (int) $params["configoptions"]["snap_num"]) {
            $name = "快照";
        }
        if (0 <= (int) $params["configoptions"]["backup_num"]) {
