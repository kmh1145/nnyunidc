...
class Shop
{
    ...
    private function _init()
    {
        ...
        if (!empty($uid)) {
            ...
            $this->save();
        } else if (!empty($shop_cookie_array)) {
        ...
        if (!empty($allConfigArr)) {
            foreach ($allConfigArr as $k => $v) {
                if ($v["option_type"] == 20 && !in_array($v["id"], $config_keys)) {
                    ...
                    } else if (!empty($configoption[$config_id])) {
                        $exists_data = \think\Db::name("product_config_options_sub")->where("config_id", $config_id)->where("hidden", 0)->where("id", $configoption[$config_id])->order("id", "asc")->find();
                        if (!empty($exists_data)) {
                            $addCartArr["configoptions"][$config_id] = $configoption[$config_id];
                        } else {
    ...
    public function removeProduct($i)
    {
        ...
        } else if (array_key_exists($i, $products_arr)) {
    ...
    public function getShopPageData($currency = 0, $pos = [])
    {
        ...
        if (!empty($products)) {
            ...
            foreach ($products as $k => $v) {
                ...
                $password = $v["password"] ?? "";
                $hostid = $v["hostid"] ?? 0;
                if (isset($v["upstream_product"])) {
                    $new_session_products[] = $v;
                } else {
                    $res = $this->checkProductToArr($pid, $billingcycle, $serverid, $configoption, $customfield, $currencyid, $qty, $os, $host, $password, $hostid);
                    if ($res["status"] == "error") {
                    } else {
                ...
                $is_zjmfapi = false;
                if ($product_data["api_type"] == "zjmf_api" && 0 < $product_data["upstream_version"] && $product_data["upstream_price_type"] == "percent") {
                    $is_zjmfapi = true;
                }
                ...
                if (in_array($product_data["type"], array_keys(config("developer_app_product_type"))) && 0 < $product_data["p_uid"]) {
                ...
                if ($billingcycle != "free") {
                    ...
                    list($itself_field, $setupfee_field) = $pricing_field;
                    $pricing_field_str = implode(",", $pricing_field);
                    $prodcut_pricing_data = $this->getProductPricing($pid, $pricing_field_str ?? "", $currencyid);
                    $product_pricing = 0 < $prodcut_pricing_data[$itself_field] ? $prodcut_pricing_data[$itself_field] : 0;
                    $product_setup_pricing = 0 < $prodcut_pricing_data[$setupfee_field] ? $prodcut_pricing_data[$setupfee_field] : 0;
                } else {
                    $product_pricing = 0;
                ...
                if (!empty($configoptions)) {
                    ...
                    foreach ($config_data as $j => $vvv) {
                        ...
                        if ($billingcycle != "free" && $vvv["hidden"] != 1) {
                            ...
                            $sub_pricing = 0 < $option_sub_pricing[$itself_field] ? $option_sub_pricing[$itself_field] : 0;
                            $sub_setup_pricing = 0 < $option_sub_pricing[$setupfee_field] ? $option_sub_pricing[$setupfee_field] : 0;
                        } else {
                            $sub_pricing = 0;
                            $sub_setup_pricing = 0;
                        }
                ...
                $apiId = getZjmfApiIdByProductId($v["pid"]);
                if ($apiId) {
                    $upstream_pid = $v["upstream_product"]["product_id"];
                    $v["upstream_product"]["is_downstream"] = 1;
                    $res = zjmfCurl($apiId, "/console/v1/product/" . $upstream_pid . "/config_option", $v["upstream_product"], 30, "POST");
                    if ($res["status"] == 200) {
                        $product_pricing = bcmul($res["data"]["price"] ?? 0, $product_data["upstream_price_value"] / 100, 2);
                        $product_setup_pricing = 0;
                        foreach ($res["data"]["preview"] as $item) {
                            $cart_products["v10_upstream_configoptions"][$item["name"]] = $item["value"];
                        }
                    }
                }
                ...
                if (0 < $hostid) {
                    ...
                    $cart_products["discount_amount"] = 0;
            ...
            if (isset($cart_data["promo"])) {
                ...
                if ($res == 1) {
                    ...
                    } else if ($promo_price_data["status"] == "error") {
    ...
    private function getSaleData($uid, $pos, &$products)
    {
        ...
        foreach ($products as $k5 => &$v5) {
            ...
            $v5["product_sale"] = 0 < $v5["product_sale"] ? $v5["product_sale"] : 0;
            $v5["product_sale_setupfee"] = 0 < $v5["product_sale_setupfee"] ? $v5["product_sale_setupfee"] : 0;
            if (in_array($k5, $pos)) {
            ...
            foreach ($configoptions_base_sale as &$v6) {
                ...
                $v6["config_rebate_price"] = 0 < $v6["config_rebate_price"] ? $v6["config_rebate_price"] : 0;
                $v6["config_rebate_setupfee"] = 0 < $v6["config_rebate_setupfee"] ? $v6["config_rebate_setupfee"] : 0;
                if (in_array($k5, $pos)) {
    ...
    private function handlePromoPrice($use_promo_data, $promo, $currency_prefix, $currency_suffix, $pos = [], $flag_promo = false)
    {
        ...
        if ($promo_flag && !empty($promo_data["appliesto"])) {
        ...
        if ($promo_flag && $promo_data["is_discount"] == 0) {
            foreach ($use_promo_data as $kk => $vv) {
        ...
        if ($promo_flag && !empty($promo_data["cycles"])) {
        ...
        if ($promo_flag && $promo_data["one_time"]) {
            $promo_produts_new = [];
    ...
    public function checkHostPassword1($password, $pid)
    {
        ...
        if ($password_basic["show"]) {
            ...
            if (strlen($password) < 6 || 20 < strlen($password)) {
    ...
    public function checkHostPassword($password, $pid)
    {
        ...
        if ($password_basic["show"]) {
            ...
            if (self::MAX_LEN < strlen($password)) {
    ...
    public function v10AddCart($addCartArr)
    {
        $cart_data = $this->cart_data;
        $cart_data["products"][] = $addCartArr;
        $this->cart_data = $cart_data;
    ...
    public function v10EditCart($i, $addCartArr)
    {
        $i = intval($i);
        $cart_data = $this->cart_data;
        $pid = $cart_data["products"][$i]["pid"];
        if (!empty($pid)) {
            $cart_data["products"][$i] = $addCartArr;
            $this->cart_data = $cart_data;
            $this->save();
        }
    }
    public function v10EditCartPage($i)
    {
        $i = intval($i);
        $cart_data = $this->cart_data;
        $config = $cart_data["products"][$i]["upstream_product"] ?? [];
        return $config;
    }
}
