...
class Cart
{
    ...
    public function getProductCycle($pid, $currencyid)
    {
        ...
        if ($paytype["pay_type"] == "onetime" && 0 <= $pricing["onetime"]) {
            ...
            $product["cycle"][] = $cycle;
        } else if ($paytype["pay_type"] == "recurring") {
    ...
    public function changeCycleToupfee($cycle)
    {
        if ($cycle == "hour") {
            ...
            $cycle = "tsetupfee";
        } else if ($cycle == "fourly") {
            $cycle = "foursetupfee";
        } else if ($cycle == "fively") {
            $cycle = "fivesetupfee";
        } else if ($cycle == "sixly") {
            $cycle = "sixsetupfee";
        } else if ($cycle == "sevenly") {
            $cycle = "sevensetupfee";
        } else if ($cycle == "eightly") {
            $cycle = "eightsetupfee";
        } else if ($cycle == "ninely") {
            $cycle = "ninesetupfee";
        } else if ($cycle == "tenly") {
            $cycle = "tensetupfee";
        }
    ...
    public function getProductDefaultConfigPrice($pid, $currency, $billingcycle, &$rebate_total, &$setupfee_total = 0)
    {
        ...
        foreach ($options as $option) {
            ...
            } else if (judgeYesNo($option["option_type"])) {
                $config_total += 0;
                $setupfee_total += 0;
                if ($option["is_rebate"] || !$edition) {
                    $rebate_total += 0;
