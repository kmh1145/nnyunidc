...
class UpperReaches
{
    ...
    public function on($id)
    {
        ...
        if ($re["control_mode"] == "ipmi") {
            $res = $this->ipmiOn($re);
        } else if ($re["control_mode"] == "dcim_client") {
    ...
    public function moduleClientButton($id)
    {
        ...
        } else if ($upper_reaches_res["control_mode"] == "ipmi") {
    ...
    public function modulePowerStatus($id)
    {
        ...
        if ($upper_reaches_res["control_mode"] == "ipmi") {
            return true;
        }
        return false;
    }
    ...
    public function dcimClientVnc($re)
    {
        ...
        if ($res["status"] == "success") {
            ...
            $result["msg"] = "VNC开启成功";
            $url = "";
            if (isset($res["data"]["vnc_link_url"]) && !empty($res["data"]["vnc_link_url"])) {
                if (request()->scheme() == "https") {
                    $url = "wss://" . $res["data"]["vnc_link_url"];
                } else {
                    $url = "ws://" . $res["data"]["vnc_link_url"];
                }
            } else {
                if ($res["data"]["ssl"]) {
