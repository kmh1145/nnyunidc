...
class Upgrade
{
    ...
    public function deleteUpgradeInvoices($hid)
    {
        ...
        $v10ApiId = getZjmfApiIdByHostId($hid);
    ...
    {
        $host_config = \think\Db::name("host_config_options")->field("configid,optionid,qty")->where("relid", $hid)->whereIn("configid", array_keys($configoptions))->select()->toArray();
        $host_config_filter = [];
        foreach ($host_config as $v) {
            ...
            } else if (judgeYesNo($option_type) && $v["qty"] == 0) {
            } else {
                $host_config_filter[$v["configid"]] = $v["optionid"];
            }
        }
        $flag = false;
        foreach ($configoptions as $k1 => $v1) {
            foreach ($host_config_filter as $k2 => $v2) {
                if ($k1 == $k2 && $v1 != $v2) {
                    $flag = true;
                }
            }
    ...
    {
        $allConfigArr = \think\Db::name("product_config_options")->alias("a")->field("a.*")->leftJoin("product_config_links b", "b.gid=a.gid")->where("b.pid", $pid)->where(function (\think\db\Query $query) use ($admin) {
            static $configoptions = NULL;
            if (!$admin) {
    ...
    public function upgradeConfigAdmin($hid, $configoptions)
    {
        ...
        if ($old_paytype["pay_type"] != "free" && $old_paytype["pay_type"] != "onetime" && $surplustime < 0) {
            $surplustime = 0;
        }
        $params = [];
        ...
        foreach ($configoptions_old as $v) {
            ...
            if (!judgeQuantity($option["option_type"])) {
                list($params["configoptions"][explode("|", $option["option_name"])[0] ?? $option["option_name"]]) = explode("|", $suboption["option_name"]);
            } else {
        ...
        foreach ($configoptions as $k => $v) {
            ...
            if (judgeQuantity($option_type)) {
                ...
                foreach ($suboptions as $kk => $vv) {
                    if (0 < $v && $vv["qty_minimum"] <= $v && $v <= $vv["qty_maximum"]) {
    ...
    public function upgradeConfigCommon($hid, $configoptions, $currencyid, $admin = false, $promo_code = "", $payment = "", $checkout = false, $percent_value = "")
    {
        ...
        $data = [];
        $desc = "";
        list($currency) = (new Currencies())->getCurrencies("id,code,prefix,suffix", $currencyid);
        ...
        if ($host["api_type"] == "zjmf_api" && 0 < $host["upstream_pid"] && $host["upstream_price_type"] == "percent") {
            $is_zjmf_api = true;
        } else {
            $is_zjmf_api = false;
        }
        ...
        $duetime = $host["nextduedate"];
        $surplustime = $duetime - time();
        if ($checkout && $old_paytype["pay_type"] != "free" && $old_paytype["pay_type"] != "onetime" && $surplustime < 0) {
            $surplustime = 0;
        }
        ...
        $old_option_total = $new_option_total = 0 < $product_pricing ? $product_pricing : 0;
        foreach ($configoptions as $k => $v) {
            ...
            if (judgeQuantity($option_type)) {
                ...
                if (!empty($options[0])) {
                    foreach ($options as $option) {
                        ...
                        if (0 <= $v && $option["min"] <= $v && $v <= $option["max"] && $min <= $v && $v <= $max) {
        ...
        if ($flag) {
            foreach ($alloption as $km => $m) {
                if (!$m["is_rebate"] && $edition) {
                } else if (judgeQuantity($m["option_type"]) && intval($m["old_qty"]) != intval($m["qty"]) || !judgeQuantity($m["option_type"]) && $m["old_suboption_id"] != $m["suboption_id"]) {
                    if ($flag["type"] == 1) {
                    ...
                    } else if ($m["is_rebate"] || !$edition) {
                        $bates = 0 < $flag["bates"] ? $flag["bates"] : 0;
                        ...
                        } else if ($new_option_total < $bates) {
                            $old_sub_pricing_sale = 0;
                            $new_sub_pricing_sale = 0;
                        } else {
                    ...
                    $saleproducts = bcadd($saleproducts, $diff, 2);
                    $diff_sub_surplustime = bcsub($diff_sub_surplustime, 0, 2);
            ...
            if ($res1["status"] == 200) {
                ...
                if (!empty($alloption)) {
                    foreach ($alloption as &$v6) {
                        if ($v6["is_discount"] && (judgeQuantity($v6["option_type"]) && intval($v6["old_qty"]) != intval($v6["qty"]) || !judgeQuantity($v6["option_type"]) && $v6["old_suboption_id"] != $v6["suboption_id"])) {
                            if ($old_upgrade_value_type == "percent") {
                                $v6["old_sub_pricing"] = $v6["old_sub_pricing"] * $old_upgrade_value / 100;
                            } else if ($old_upgrade_value_type == "fixed") {
                                $v6["old_sub_pricing"] = 0 < $v6["old_sub_pricing"] - $old_upgrade_value ? $v6["old_sub_pricing"] - $old_upgrade_value : 0;
                            }
                        }
                        if ($old_promo_data && isset($v6["upgrade_item"]["recurring_change"])) {
        ...
        if ($promo_code) {
            ...
            if ($result["status"] == 200) {
                $promo = $result["data"];
                $upgrade_value = 100 <= $promo["value"] ? 100 : (0 < $promo["value"] ? $promo["value"] : 0);
                $upgrade_value_type = $promo["type"];
                $promoqualifies = true;
                if (!empty($alloption)) {
                    foreach ($alloption as &$v7) {
                        if ($v7["is_discount"] && (judgeQuantity($v7["option_type"]) && intval($v7["old_qty"]) != intval($v7["qty"]) || !judgeQuantity($v7["option_type"]) && $v7["old_suboption_id"] != $v7["suboption_id"])) {
                            ...
                            } else if ($upgrade_value_type = "fixed") {
                        ...
                        if ($promo && $promo["recurring"] == 1 && isset($v7["upgrade_item"]["recurring_change"])) {
        ...
        if ($promoqualifies || $old_promoqualifies) {
            if (!empty($alloption)) {
                foreach ($alloption as $k3 => $v3) {
                    ...
                    if ($res_is_discount && ($promoqualifies || $old_promoqualifies) && !empty($v3["upgrade_item"])) {
        ...
        $saleproducts = 0 < $saleproducts ? bcsub($saleproducts, 0, 2) : 0;
            ...
            try {
                ...
                foreach ($alloption_filter as $kkk => $vvv) {
                    ...
                    if (!empty($upgrade_item_tmp)) {
                        ...
                        $invoice_items_data = [];
                        if (0 <= $upgrade_item_tmp["amountdue"] && $invoice_id) {
                            ...
                            if ($upgrade_item_tmp["item_discount"] && 0 <= $upgrade_item_tmp["item_discount"] && 0 <= $diff_pricing) {
                                $invoice_items_data[] = ["invoice_id" => $invoice_id, "uid" => $uid, "type" => "promo", "rel_id" => intval($upgrade_id), "description" => $upgrade_item_tmp["discount_description"] ?? "", "amount" => -1 * $upgrade_item_tmp["item_discount"], "due_time" => time(), "payment" => $payment];
                            }
                        }
                ...
                if ($flag && !empty($upgrade_ids[0])) {
                    $invoice_items_data = ["invoice_id" => $invoice_id, "uid" => $uid, "type" => "discount", "rel_id" => intval($upgrade_ids[0]), "description" => "客戶折扣", "amount" => -1 * $data["saleproducts"], "due_time" => time(), "payment" => $payment];
    ...
    public function checkUpgradePromo($promo_code, $hid, $new_pid = "", $new_billingcycle = "", $type = "option", $orgin = false)
    {
        ...
        if ($promo["expiration_time"] != "0" && $promo["expiration_time"] < time()) {
        ...
        } else if (count($appliesto) && $appliesto[0] && !in_array($pid, $appliesto)) {
            return ["status" => 400, "msg" => lang("PROMO_CODE_INAPPLICABILITY_PRODUCT")];
        }
        ...
        } else if (count($cycle) && $cycle[0] && !in_array($billingcycle, $cycle)) {
            return ["status" => 400, "msg" => lang("PROMO_CODE_CAN_NOT_APPLY_BILLINGCYLCE")];
        }
        if ($type == "option" && $promo["lifelong"] == 0 && !$orgin) {
            return ["status" => 400, "msg" => lang("PROMO_CODE_CAN_NOT_UPGRADS_OPTIONS")];
        }
        if ($type == "product" && empty($appliesto[0])) {
    ...
    public function doUpgrade($upgrade_id)
    {
        ...
        if (!empty($upgrade) && $upgrade["status"] != "Completed") {
            ...
            if ($upgrade["type"] == "product") {
                ...
                if ($new_product["api_type"] == "zjmf_api" && 0 < $new_product["upstream_pid"]) {
                ...
                foreach ($olds as $v) {
                    if (judgeOs($v["option_type"])) {
                        list($os_id_old) = explode("|", $v["option_name"]);
                ...
                if ($v10Api = getZjmfApiIdByHostId($upgrade["relid"])) {
                } else {
                    $is_config = true;
                    $original_value = $upgrade["original_value"];
                    $new_value = $upgrade["new_value"];
                    list($config_id) = explode("=>", $original_value);
                    ...
                    if (!$count) {
                        ...
                        } else if (judgeYesNo($option_type)) {
                    ...
                    } else if (judgeQuantity($option_type) || judgeYesNo($option_type)) {
    ...
    public function getProductUpgradeConfig($hid, $pid, $billingcycle, $currencyid)
    {
        ...
        foreach ($newoptions as $kkk => $newoption) {
            ...
            if ($newoptiontype == "3") {
                ...
                if (0 < $newsuboptionprice) {
                    $newsuboptionname .= " " . $newsuboptionprice;
                }
                $setupvalue = 0 < $newsuboptionsetupfee ? " + " . $newsuboptionsetupfee . " " . lang("ORDERS_SETUPFEE") : "";
            ...
            } else if (judgeQuantity($newoptiontype)) {
                $newsuboptions = \think\Db::name("product_config_options_sub")->alias("pcos")->field("")->leftJoin("pricing p", "p.relid = pcos.id")->where("p.type", "configoptions")->where("p.currency", $currencyid)->where("pcos.config_id", $newoptionid)->order("pcos.sort_order ASC")->order("pcos.id asc")->limit(1)->select();
                $exist = false;
                foreach ($newsuboptions as $newsuboption) {
                    ...
                    if (0 < $newoptionqtymin && $selectedvalue < $newoptionqtymin) {
                        $selectedvalue = $newoptionqtymin;
                        if ($newsuboptionqtymin == $newoptionqtymin) {
                            $selectedqty = $selectedvalue;
                            $selsubid = $newsuboptionid;
                    ...
                    if (0 < $newoptionqtymax && $newoptionqtymax < $selectedvalue) {
                ...
                if (judgeQuantityStage($newoptiontype)) {
                    ...
                    list($selrecurring, $selsetup) = $sum;
                }
            } else {
                ...
                foreach ($newsuboptions as $newsuboption) {
                    ...
                    if ($newsuboptionid == $selectedvalue || !$selectedvalue && !$newsuboptionhidden) {
                        ...
                        if (!$foundpreselectedvalue && 0 < count($suboptionsarray)) {
    ...
    public function upgradeProductCommon($hid, $new_pid, $new_billingcycle, $currency = "", $promo_code = "", $payment = "", $checkout = false, $percent_value = "")
    {
        ...
        list($currency) = (new Currencies())->getCurrencies("id,code,prefix,suffix", $currency_id);
        ...
        if ($checkout && $old_paytype["pay_type"] != "free" && $old_paytype["pay_type"] != "onetime" && $old_surplus_time < 0) {
            $old_surplus_time = 0;
        }
        ...
        $is_zjmf_pai = false;
        if ($new_product["api_type"] == "zjmf_api" && 0 < $new_product["upstream_pid"] && $new_product["upstream_price_type"] == "percent") {
            $is_zjmf_pai = true;
        }
        ...
        foreach ($new_option_prices as $new_option_price) {
            if ($new_option_price["hidden"] == 0) {
                ...
                if ($new_option_price["is_rebate"] && $flag["type"] == 1) {
                    $bates = $flag["bates"] / 100;
        ...
        if ($flag["type"] == 1) {
            ...
            $saleproducts = 0 < bcsub($new_amount, $bates) ? $bates : $new_amount;
            $new_amount = 0 < bcsub($new_amount, $bates) ? bcsub($new_amount, $bates) : 0;
        ...
        } else if ($new_billingcycle == "onetime" || $new_billingcycle == "free") {
        ...
        if ($checkout) {
            ...
            try {
                ...
                if (isset($last_upgrade_invoice["credit"]) && 0 < $last_upgrade_invoice["credit"]) {
                    ...
                    credit_log(["uid" => $uid, "desc" => "Credit Removed from Invoice #" . $last_upgrade_invoice_id, "amount" => -1 * $credit, "relid" => $last_upgrade_invoice_id]);
                }
                ...
                if ($invoice_id) {
                    ...
                    if ($flag) {
                        $invoice_items_data[] = ["invoice_id" => $invoice_id, "uid" => $uid, "type" => "discount", "rel_id" => $upgrade_id, "description" => "客戶折扣", "amount" => -1 * $saleproducts, "due_time" => time(), "payment" => $payment];
                    }
                    if (0 < $discount) {
