...
class Invoices
{
    ...
    public function useCreditPay($invoice_id)
    {
        ...
        } else if ($info["subtotal"] - $info["total"] <= $info["credit"]) {
            \think\Db::startTrans();
            try {
    ...
    public function processPaidInvoiceFinal($invoiceid, $email = true)
    {
        ...
        foreach ($invoice_item_data as $key => $val) {
            ...
            $amount = $val["amount"];
            if ($type == "host") {
                $host = \think\Db::name("host")->alias("a")->field("a.id,a.uid,a.orderid,o.amount,a.productid,a.domainstatus,a.nextduedate,a.regdate,b.welcome_email,b.type,c.email,a.billingcycle,b.pay_type,b.api_type,a.upstream_configoption")->leftJoin("products b", "a.productid=b.id")->leftJoin("clients c", "a.uid=c.id")->join("orders o", "a.orderid=o.id")->where("a.id", $relid)->find();
                $v10ApiId = getZjmfApiIdByHostId($relid);
                if (!empty($host)) {
                    ...
                    if ($host["billingcycle"] == "onetime" || $host["billingcycle"] == "free") {
                        ...
                        if ($v10ApiId) {
                            $nextduedate = time() + json_decode($host["upstream_configoption"], true)["duration"] ?? 0;
                        } else {
