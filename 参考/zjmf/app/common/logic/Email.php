...
class Email
{
    ...
    public function sendEmail($templateId, $relid, $ip = "")
    {
        ...
        if ($type === "invoice") {
            ...
            if (isset($invoice["rel_id"]) && 0 < $invoice["rel_id"]) {
    ...
    protected function getContent($content, $type, $relid)
    {
        ...
        if ($type == "admin") {
            ...
            $result = $content;
        } else if ($type == "product" || $type == "invoice" || $type == "credit_limit") {
            ...
            $replaceAdd = [$host["product_name"], $host["domain"], $host["username"], $host["dedicatedip"], cmf_decrypt($host["password"]), $host["os"], $host["assignedips"], date("Y-m-d", $host["create_time"]), 0 < $host["nextduedate"] ? date("Y-m-d", $host["nextduedate"]) : "永久", config("billing_cycle")[$host["billingcycle"]], date("Y-m-d", $host["create_time"]), $host["id"], $host["prefix"] . $host["amount"] . $host["suffix"], !empty($host["paid_time"]) ? date("Y-m-d", $host["paid_time"]) : date("Y-m-d"), date("Y-m-d", $terminate_time), $invoice["id"], $invoice["total"]];
            ...
            switch ($value) {
                ...
                case "CLIENT_STATUS":
                    if ($client_info["status"] == 1) {
                        $client_status = "激活";
                    } else if ($client_info["status"] == 0) {
                        $client_status = "未激活";
                    } else if ($client_info["status"] == 2) {
                        $client_status = "关闭";
                    } else {
                        $client_status = "未知";
                    }
