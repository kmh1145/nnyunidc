...
class ConfigOptions
{
        ...
        if (!empty($allConfigArr[0])) {
            foreach ($allConfigArr as $k => $v) {
                if (!in_array($v["id"], $config_keys)) {
                    ...
                    } else if ($configoptions[$config_id]) {
            ...
            foreach ($configgroups as $okey => $option) {
                if (!empty($option)) {
                    ...
                    if ($option["option_type"] == 3) {
                        $suboptions = \think\Db::name("product_config_options_sub")->where("config_id", $cid)->where(function (\think\db\Query $query) {
                            static $admin = NULL;
                            if (!$admin) {
                    ...
                    if ($option["option_type"] == 5) {
                        foreach ($suboptions as $subkey => $suboption) {
                            if (!empty($suboption)) {
                                ...
                                $suboption["option_name"] = explode("|", $suboption_name)[1] ? explode("|", $suboption_name)[1] : $suboption_name;
                                ...
                                if (strtolower($os) == "windows") {
                                    $base = 1;
                                } else if (strtolower($os) == "centos") {
                                    $base = 2;
                                } else if (strtolower($os) == "ubuntu") {
                                    $base = 3;
                                } else if (strtolower($os) == "debian") {
                                    $base = 4;
                                } else if (strtolower($os) == "esxi") {
                                    $base = 5;
                                } else if (strtolower($os) == "xenserver") {
                                    $base = 6;
                                } else if (strtolower($os) == "freebsd") {
                                    $base = 7;
                                } else if (strtolower($os) == "fedora") {
                                    $base = 8;
                                } else {
                                    $base = 9;
                                }
    ...
    public function configShow($configInfo, $user_currcy_id, $billingcycle, $uid = 0, $upgrade = false)
    {
        ...
        foreach ($configInfo as $key => $val) {
            ...
            } else if (judgeNoc($val["option_type"])) {
                foreach ($sub as $k => $v) {
                    $area = [];
            ...
            if (0 < count($qty_minimum)) {
                $config_array[$i]["qty_minimum"] = min($qty_minimum);
            }
            if (0 < count($qty_maximum)) {
                ...
                foreach ($options as $okey => $option) {
                    if ($option["option_type"] == 20 && !in_array($option["id"], $host_config)) {
                        ...
                        if (empty($suboptions)) {
                            ...
                            } else if (judgeQuantity($option["option_type"])) {
    ...
    public function updateConfig($hostid, $configoption, $new_productid = 0, $product_info = [])
    {
        ...
        if ($pid != $new_productid && 0 < $new_productid) {
            $host_del = [];
    ...
    public function getOs($configid, $subid)
    {
        ...
        if (explode("^", $suboption["option_name"])[1]) {
            list($os) = explode("^", $suboption["option_name"]);
            list($os_url) = explode("^", $suboption["option_name"]);
