...
class DcimCloud
{
    ...
    public function moduleClientAreaDetail($id, $key, $api_url = "", $v10 = false)
    {
        ...
        if ($key == "snapshot") {
            ...
            if ($v10) {
                return ["status" => 200, "msg" => "请求成功", "data" => ["list" => $res["vars"]["list"], "disk" => $cloud["data"]["disk"], "support_snap" => $support_snap, "support_backup" => $support_backup, "host_type" => $host_type, "backup_num" => isset($params["configoptions"]["backup_num"]) ? (int) $params["configoptions"]["backup_num"] : -1, "snap_num" => isset($params["configoptions"]["snap_num"]) ? (int) $params["configoptions"]["snap_num"] : -1]];
            }
        } else if ($key == "setting") {
    ...
    public function remoteInfo($id)
    {
        ...
        if ($res["status"] == "success" || $res["status"] == 200) {
            $result["status"] = 200;
            $result["msg"] = "获取成功";
            $result["data"] = ["rescue" => (int) $res["data"]["rescue"]];
        } else {
            $result["status"] = 400;
    ...
    public function reinstall($id, $os, $port = 0)
    {
        ...
        } else if ($system_disk_size_config_options["option_type"] == 14 || $system_disk_size_config_options["option_type"] == 19) {
    ...
    public function buyFlowPacket($params)
    {
        ...
        if ($res["status"] == "success") {
            ...
            if ($info["status"] == "success") {
                ...
                } else if ($info["data"]["meta"]["traffic_type"] == 2) {
                    $usage = round($info["data"]["info"]["30_day"]["send"] / pow(1024, 3), 2);
                } else if ($info["data"]["meta"]["traffic_type"] == 3) {
        ...
        if ($res["status"] == "success") {
            ...
            if ($res["status"] == "success") {
                if ($res["data"]["status"] == "suspend") {
                    $result["status"] = 200;
                    $result["msg"] = "暂停成功";
                } else if ($res["data"]["status"] == "task" && $res["data"]["task"] == "suspend") {
                    $result["status"] = 200;
                    $result["msg"] = "暂停成功";
                } else {
                    $result["status"] = 400;
    ...
    public function upgrade($id, $old)
    {
        ...
        if (isset($params["configoptions_upgrade"]["memory"]) && ($params["configoptions_upgrade"]["memory"] / 1024 != $detail["memory"] || $params["configoptions_upgrade"]["memory"] != $old["configoptions"]["memory"])) {
            ...
            } else if ($memory_config_options["option_type"] == 9 || $memory_config_options["option_type"] == 17) {
                list($sub_option) = explode("|", $memory_config_options["sub_option"]);
        ...
        if (isset($params["configoptions_upgrade"]["data_disk_size"]) && $params["configoptions_upgrade"]["data_disk_size"] != $old["configoptions"]["data_disk_size"]) {
            ...
            } else if ($data_disk_size_config_options["option_type"] == 14 || $data_disk_size_config_options["option_type"] == 19) {
                list($sub_option) = explode("|", $data_disk_size_config_options["sub_option"]);
                if (!empty($sub_option)) {
        ...
        if (isset($params["configoptions_upgrade"]["system_disk_io_limit"]) && $params["configoptions_upgrade"]["system_disk_io_limit"] != $old["configoptions"]["system_disk_io_limit"] && !empty($params["configoptions"]["system_disk_io_limit"])) {
            if (!$needOff) {
                $needOff = $waitOff();
            }
        ...
        if (isset($params["configoptions_upgrade"]["data_disk_io_limit"]) && $params["configoptions_upgrade"]["data_disk_io_limit"] != $old["configoptions"]["data_disk_io_limit"] && !empty($params["configoptions"]["data_disk_io_limit"])) {
            if (!$needOff) {
                $needOff = $waitOff();
            }
    ...
    public function createAccount($hostid)
    {
        ...
        if ($params["type"] == "dcimcloud") {
            ...
            $post_data["gpu_num"] = isset($params["configoptions"]["gpu_num"]) ? (int) $params["configoptions"]["gpu_num"] : NULL;
            ...
            if ($post_data["network_type"] == "vpc") {
                ...
                } else if (!empty($post_data["node_group"])) {
            ...
            if ($data_disk_size_config_options["option_type"] == 1 || $data_disk_size_config_options["option_type"] == 13) {
                ...
                } else if (!empty($sub_option) && !empty($params["configoptions"]["data_disk_size"])) {
