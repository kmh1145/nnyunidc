...
class Host
{
    ...
    public function createFinal($id, $ip = "")
    {
        ...
        if (getZjmfApiIdByProductId($host["productid"])) {
            $host["api_type"] = "v10";
        }
        if ($host["api_type"] == "zjmf_api") {
            ...
            if ($module_res["status"] == 200) {
                ...
                if ($module_res2["status"] == 200) {
                    ...
                    if (!empty($module_res2["invoiceid"])) {
                        ...
                        if ($module_res["status"] == 200) {
                            ...
                            } else if ($module_res["status"] == 1001) {
                                $module_res["status"] = 200;
                                $hostid = $module_res["data"]["hostid"][0];
                                $update["dcimid"] = $hostid;
                                \think\Db::name("host")->where("id", $id)->update($update);
                ...
                } else if ($module_res2["status"] == 400) {
        ...
        } else if ($host["api_type"] == "resource") {
        ...
        } else if ($host["api_type"] == "v10") {
            if (function_exists("v10HostCreate")) {
                $module_res = v10HostCreate($id);
                if ($module_res["status"] == 400) {
                    $module_res["msg"] = "v10：" . $module_res["msg"];
                }
            }
        } else if (!empty($host["server_group"])) {
            if ($host["type"] == "dcim") {
                $dcim = new Dcim();
    ...
    public function suspend($id, $reason_type = "other", $reason = "", $send = 0)
    {
        ...
        if (!$this->is_admin && request()->uid != $host["uid"]) {
            $result["status"] = 406;
            $result["msg"] = "不能执行该操作";
            return $result;
        }
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            if (function_exists("v10HostSuspend")) {
                $module_res = v10HostSuspend($id, $reason);
                if ($module_res["status"] == 400) {
                    $module_res["msg"] = "v10：" . $module_res["msg"];
                }
            }
            ...
            if ($host["type"] == "dcim") {
                ...
                if ($module_res["no_support_function"]) {
                    $module_res = ["status" => 200];
                }
            }
        } else {
    ...
    public function unsuspend($id, $send = 0, $type = "", $force_success = false)
    {
        ...
        if (!empty($type)) {
            list($host["suspendreason"]) = explode("-", $host["suspendreason"]);
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            if (function_exists("v10HostUnsuspend")) {
                $module_res = v10HostUnsuspend($id);
                if ($module_res["status"] == 400) {
                    $module_res["msg"] = "v10：" . $module_res["msg"];
                }
            }
        ...
        if ($module_res["status"] == "success" || $module_res["status"] == 200 || $force_success) {
            ...
            list($suspendreason) = explode("-", $host["suspendreason"]);
    ...
    public function terminate($id, $cron = "")
    {
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            if (function_exists("v10HostTerminate")) {
                $module_res = v10HostTerminate($id);
                if ($module_res["status"] == 400) {
                    $module_res["msg"] = "v10：" . $module_res["msg"];
                }
            }
    ...
    public function sync($id)
    {
        ...
        if ($apiId = getZjmfApiIdByProductId($host["productid"])) {
            $host["api_type"] = "v10";
        }
        if ($host["api_type"] == "zjmf_api") {
            ...
            if ($module_res["status"] == 200) {
                ...
                $update["bwlimit"] = (int) $module_res["data"]["host_data"]["bwlimit"];
                $update["bwusage"] = (double) $module_res["data"]["host_data"]["bwusage"];
                $update["username"] = $module_res["data"]["host_data"]["username"] ?? "";
                $update["password"] = cmf_encrypt($module_res["data"]["host_data"]["password"]);
                $update["port"] = (int) $module_res["data"]["host_data"]["port"];
            ...
            $module_res = zjmfCurl($apiId, "/console/v1/host/" . $host["dcimid"] . "/sync", [], 30, "GET");
            if ($module_res["status"] == 200) {
                $params = $module_res["data"];
                $update = ["username" => $params["username"], "password" => cmf_encrypt($params["password"]), "os" => $params["os"], "os_url" => $params["os_url"] ?: "", "dedicatedip" => $params["dedicatedip"], "assignedips" => $params["assignedips"], "port" => (int) $params["port"], "suspendreason" => $params["suspendreason"]];
    ...
    public function on($id)
    {
        if (is_array($id)) {
            ...
            if (!$this->is_admin && ($host["domainstatus"] != "Active" || request()->uid != $host["uid"])) {
                $result["status"] = 406;
                $result["msg"] = "不能执行该操作";
                return $result;
            }
    ...
    public function status($id, $format = false)
    {
        if (is_array($id)) {
            ...
            $host = \think\Db::name("host")->alias("a")->field("a.id,a.productid,a.domainstatus,a.uid,a.dcimid,b.server_type,b.type,b.api_type,b.zjmf_api_id,c.email,b.server_group,a.dcimid")->leftJoin("products b", "a.productid=b.id")->leftJoin("clients c", "a.uid=c.id")->where("a.id", $id)->find();
            if (empty($host)) {
                $result["status"] = 406;
                $result["msg"] = "ID错误";
                return $result;
            ...
            if ($host["api_type"] == "zjmf_api") {
                ...
                if ($host["type"] == "dcim") {
                    ...
                    if ($format && $module_res["status"] == 200 && isset($module_res["data"]["power"])) {
                    ...
                    if ($format && ($module_res["status"] == 200 || $module_res["status"] == "success") && !isset($module_res["data"]["status"])) {
                        $module_res = [];
    ...
    public function changePackage($id, $params, $is_config = false, $is_upstream_product = false, $upgrade_id = 0)
    {
        ...
        if ($v10Api = getZjmfApiIdByHostId($id)) {
            $upgrade = \think\Db::name("upgrades")->where("id", $upgrade_id)->find();
            $param = json_decode($upgrade["new_value"], true);
            $upgradeType = $param["upgrade_type"];
            unset($param["upgrade_type"]);
            if (getZjmfApiProductTypeByProductId($host["productid"]) == "dcimcloud") {
                if ($upgradeType == "common_config") {
                    $result = zjmfCurl($v10Api, "/console/v1/mf_cloud/" . $host["dcimid"] . "/common_config/order", $param, 30, "POST");
                } else if ($upgradeType == "backup_config") {
                    $result = zjmfCurl($v10Api, "/console/v1/mf_cloud/" . $host["dcimid"] . "/backup_config/order", $param, 30, "POST");
                } else if ($upgradeType == "disk") {
                    $result = zjmfCurl($v10Api, "/console/v1/mf_cloud/" . $host["dcimid"] . "/disk/resize/order", $param, 30, "POST");
                } else if ($upgradeType == "new_disk") {
                    $result = zjmfCurl($v10Api, "/console/v1/mf_cloud/" . $host["dcimid"] . "/disk/order", $param, 30, "POST");
                } else if ($upgradeType == "image") {
                    $result = zjmfCurl($v10Api, "/console/v1/mf_cloud/" . $host["dcimid"] . "/image/order", $param, 30, "POST");
                } else if ($upgradeType == "ip_num") {
                    $result = zjmfCurl($v10Api, "/console/v1/mf_cloud/" . $host["dcimid"] . "/ip_num/order", $param, 30, "POST");
                } else if ($upgradeType == "flow_packet") {
                    $result = zjmfCurl($v10Api, "/console/v1/host/" . $host["dcimid"] . "/flow_packet_order", $param, 30, "POST");
                } else if ($upgradeType == "recommend_config") {
                    $result = zjmfCurl($v10Api, "/console/v1/mf_cloud/" . $host["dcimid"] . "/recommend_config/order", $param, 30, "POST");
                }
            } else if ($upgradeType == "common_config") {
                $result = zjmfCurl($v10Api, "/console/v1/mf_dcim/" . $host["dcimid"] . "/common_config/order", $param, 30, "POST");
            } else if ($upgradeType == "backup_config") {
                $result = zjmfCurl($v10Api, "/console/v1/mf_dcim/" . $host["dcimid"] . "/backup_config/order", $param, 30, "POST");
            } else if ($upgradeType == "disk") {
                $result = zjmfCurl($v10Api, "/console/v1/mf_dcim/" . $host["dcimid"] . "/disk/resize/order", $param, 30, "POST");
            } else if ($upgradeType == "new_disk") {
                $result = zjmfCurl($v10Api, "/console/v1/mf_dcim/" . $host["dcimid"] . "/disk/order", $param, 30, "POST");
            } else if ($upgradeType == "image") {
                $result = zjmfCurl($v10Api, "/console/v1/mf_dcim/" . $host["dcimid"] . "/image/order", $param, 30, "POST");
            } else if ($upgradeType == "ip_num") {
                $result = zjmfCurl($v10Api, "/console/v1/mf_dcim/" . $host["dcimid"] . "/ip_num/order", $param, 30, "POST");
            } else if ($upgradeType == "flow_packet") {
                $result = zjmfCurl($v10Api, "/console/v1/host/" . $host["dcimid"] . "/flow_packet_order", $param, 30, "POST");
            }
            if ($result["status"] == 200) {
                $orderId = $result["data"]["id"] ?? "";
                $result = zjmfCurl($v10Api, "/console/v1/order/" . $orderId, [], 30, "GET");
                if ($result["status"] == 200) {
                    if (isset($result["data"]["order"]["status"]) && $result["data"]["order"]["status"] == "Paid") {
                        $recurring_change = $upgrade["recurring_change"];
                        \think\Db::name("host")->where("id", $id)->setInc("amount", $recurring_change);
                    } else {
                        $payData = ["id" => $orderId, "gateway" => "credit"];
                        $result = zjmfCurl($v10Api, "/console/v1/pay", $payData, 30, "POST");
                        if ($result["status"] == 200 && $result["code"] == "Paid") {
                            $recurring_change = $upgrade["recurring_change"];
                            \think\Db::name("host")->where("id", $id)->setInc("amount", $recurring_change);
                        }
                    }
        ...
        } else if ($host["api_type"] == "zjmf_api" && $is_config) {
            $post_data = [];
        ...
        } else if ($host["api_type"] == "zjmf_api" && $is_upstream_product) {
            $post_data = [];
            $post_data["hid"] = $host["dcimid"];
            $post_data["pid"] = $host["upstream_pid"];
            $post_data["billingcycle"] = $host["billingcycle"];
        ...
        } else if ($host["api_type"] == "resource" && $is_config) {
            if (function_exists("resourceCurl")) {
                ...
                if ($module_res["status"] == 200) {
                    ...
                    } else if ($res_products["price_type"] == "handling") {
        ...
        } else if ($host["api_type"] == "resource" && $is_upstream_product) {
            $res_products = \think\Db::name("host")->alias("a")->field("b.percent_value,b.price_type")->leftJoin("res_products b", "a.productid=b.productid")->where("a.id", $id)->find();
            $post_data = [];
            $post_data["hid"] = $host["dcimid"];
            $post_data["pid"] = $host["upstream_pid"];
    ...
    public function renew($id)
    {
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            if (function_exists("v10HostRenew")) {
                $module_res = v10HostRenew($id);
                if ($module_res["status"] == 400) {
                    $module_res["msg"] = "v10：" . $module_res["msg"];
                }
            }
    ...
    public function createTicket($id, $ticketid)
    {
        ...
        if ($host["api_type"] == "zjmf_api") {
        } else if ($host["api_type"] == "normal" && !empty($host["server_group"])) {
            if ($host["type"] == "dcim") {
            } else if ($host["type"] == "dcimcloud") {
            } else {
    ...
    public function replyTicket($id, $reply_data)
    {
        ...
        if ($host["api_type"] == "zjmf_api") {
            ...
            if ($host["type"] == "dcim") {
                ...
                $params["ticket_reply"] = ["id" => (int) $reply_data["id"], "tid" => $reply_data["tid"], "create_time" => $reply_data["create_time"], "content" => $reply_data["content"], "attachment" => []];
