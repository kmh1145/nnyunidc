...
class PluginModel extends think\Model
{
    ...
    {
        if (!in_array($type, ["gateways", "addons", "certification", "firewall", "oauth", "sms", "automatic", "mail", "captcha"])) {
            return false;
        }
        if ($type == "gateways") {
            ...
            foreach ($plugins as $k => &$v) {
                ...
                if (!class_exists($class)) {
                    ...
                    foreach ($methods as $m) {
                        ...
                        } else if (strpos($m, "CnTemplate") !== false || $m == "sendCnSms") {
                            $type["cn"] = ["name" => "cn", "name_zh" => "国内"];
                        } else if (strpos($m, "CnProTemplate") !== false || $m == "sendCnProSms") {
        ...
        foreach ($plugins as $key => &$val) {
            ...
            if (!isset($result_data[$key])) {
                ...
                } else if (time() < $result_data[$key]["nextduedate"]) {
                    $val->update_disable = 0;
                }
            }
        }
