...
class Cron extends think\console\Command
{
    ...
    public function execute(\think\console\Input $input, \think\console\Output $output)
    {
        ...
        if (time() < 86400 - intval(configuration("cron_current_time_first_once")) + $config["cron_day_start_time"]) {
            return NULL;
        }
        ...
        if (configuration("cron_last_run_lock_status")) {
            ...
            if (600 <= time() - configuration("cron_last_run_time_over_in")) {
    ...
    {
        $this_time = time();
        if (date("G") < $config["cron_day_start_time"]) {
            return NULL;
        }
        $time_day = strtotime(date("Y-m-d")) + intval($config["cron_day_start_time"]) * 60 * 60;
        if (time() < $time_day || $time_day + 900 < time()) {
            return NULL;
        }
    ...
    public function host($config)
    {
        ...
        if (!empty($config["cron_host_suspend"])) {
            ...
            if (!empty($host[0])) {
                foreach ($host as $v) {
                    if (0 < $v["overideautosuspend"] && $now < $v["overidesuspenduntil"]) {
                    } else {
    ...
    public function invoice($config)
    {
        ...
        if (!empty($host[0])) {
            ...
            } else if (configuration("no_auto_apply_credit") == 1) {
                foreach ($host as $v) {
                    if (!checkHostIsCancel($v["id"])) {
                        $result = $invoice->createRenew($v["id"], $v["initiative_renew"], true);
                        if ($result["status"] == 200) {
        ...
        if (0 < $config["cron_invoice_pay_email"] && 0 < $config["cron_invoice_unpaid_email"]) {
        ...
        if (!empty($config["cron_invoice_pay_email"])) {
            if (0 < $config["cron_invoice_third_overdue_email"]) {
                $this->invoiceDueSend($config, 2);
            }
            if (0 < $config["cron_invoice_second_overdue_email"]) {
                $this->invoiceDueSend($config, 1);
            }
            if (0 < $config["cron_invoice_first_overdue_email"]) {
    ...
    public function order($config)
    {
        ...
        if ($cron_order_unpaid_time_high == 1 && 0 < $day) {
            $before_day_start_time = strtotime("-" . $day . " days", strtotime(date("Y-m-d")));
            $before_day_end_time = strtotime("+1 days -1 seconds", $before_day_start_time);
    ...
    public function unCertifi($config)
    {
        ...
        if (!empty($use) && 0 <= $day && $certifi_open == 1) {
            $before_day_start_time = strtotime("-" . $day . " days", strtotime(date("Y-m-d")));
            $before_day_end_time = strtotime("+1 days -1 seconds", $before_day_start_time);
                ...
                if ($v["http_code"] == 200 && $v["status"] == 200 && empty($v["data"]["status"]) && !empty($v["data"])) {
                    ...
                    if (!empty($area)) {
                        ...
                        foreach ($area as $kk => $vv) {
                            $area_name[$vv["id"]] = (string) $vv["name"] ?? "";
                        }
    ...
    public function syncUpstreamProductInfo1()
    {
        ...
        if (!empty($products[0])) {
            ...
            foreach ($products as $product) {
                ...
                if ($res["data"]["products"] && $product["upstream_version"] < $res["data"]["products"]["location_version"]) {
                    if ($product["upstream_price_type"] == "percent") {
                        ...
                        } else if ($pay_type["pay_type"] == "recurring") {
    ...
    public function moduleCron($type = "FiveMinuteCron")
    {
        ...
        foreach ($module as $v) {
            ...
            } else if (file_exists($dir2 . $v . "/" . $v . ".php")) {
    ...
    {
        $datetime = date("Ymd");
        $idata = ["cron_type" => (int) $cron_type, "active_type" => (int) $active_type, "status" => (int) $status, "create_time" => time(), "datetime" => $datetime, "unique_tab" => $datetime . $cron_type . $unique_id];
    ...
    private function getAppHeat()
    {
        ...
        if ($app_get_heat_time + 86400 <= strtotime($date) && $now == $date) {
    ...
    private function v10ProductInputUpdate()
    {
        $products = \think\Db::name("products")->alias("p")->field("p.id,p.upstream_pid,p.description,p.zjmf_api_id")->leftJoin("zjmf_finance_api zfa", "p.zjmf_api_id=zfa.id")->where("zfa.type", "v10")->select()->toArray();
        foreach ($products as $product) {
