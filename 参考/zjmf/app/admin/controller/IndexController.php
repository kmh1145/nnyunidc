...
class IndexController extends AdminBaseController
{
    ...
    private function searchCeriClients($value, $flag = 0)
    {
        ...
        $datacl = \think\Db::name("clients")->field("c.id,c.id as url,c.email as values1,c.username as values2,c.email as values3,c.status")->alias("c")->leftJoin("certifi_log cl", "cl.uid=c.id")->where("cl.status=2")->where(function (\think\db\Query $query) use ($param) {
            static $value = NULL;
            foreach ($param as $v) {
                $query->whereOr($v, "like", "%" . $value . "%");
            }
    ...
    private function searchOrders($value, $key, $flag = 0)
    {
        ...
        $data = \think\Db::name("orders")->alias("o")->join("host h", "o.id=h.orderid")->join("products p", "p.id=h.productid")->join("product_groups pg", "pg.id=p.gid")->join("clients c", "c.id=o.uid")->field("c.id,o.id as url,p.name as values1,h.dedicatedip as values2,o.status,c.username as values3")->where(function (\think\db\Query $query) use ($param) {
            static $value = NULL;
            foreach ($param as $v) {
                $query->whereOr($v, "like", "%" . $value . "%");
            }
    ...
    private function searchInvoice($value, $key, $flag = 0)
    {
        ...
        $data = \think\Db::name("Invoices")->alias("i")->join("clients c", "c.id=i.uid")->field("c.id,i.id as url,i.id as values2,c.username as values3,i.status")->where(function (\think\db\Query $query) use ($param) {
            static $value = NULL;
            foreach ($param as $v) {
                $query->whereOr($v, "like", "%" . $value . "%");
            }
    ...
    private function searchTickets($value, $key, $flag = 0)
    {
        ...
        $data = \think\Db::name("Ticket")->alias("t")->join("ticket_department td", "td.id=t.dptid")->join("clients c", "c.id=t.uid")->field("c.id,t.id as url,t.tid as values2,t.status,t.title,c.username as values3")->where(function (\think\db\Query $query) use ($param) {
            static $value = NULL;
            foreach ($param as $v) {
                $query->whereOr($v, "like", "%" . $value . "%");
            }
    ...
    private function searchClients_left($value, $key, $flag = 0, $page, $limit)
    {
        ...
        $total = $total->where(function (\think\db\Query $query) use ($param) {
            static $value = NULL;
            foreach ($param as $v) {
    ...
    private function searchProducts_left($value, $key, $flag = 0, $page, $limit)
    {
        ...
        $count = $count->where($where)->where(function (\think\db\Query $query) use ($param) {
            static $value = NULL;
            foreach ($param as $v) {
                $query->where($v, "like", "%" . $value . "%");
            }
        ...
        $total = $total->where($where)->where(function (\think\db\Query $query) use ($param) {
            static $value = NULL;
            foreach ($param as $v) {
                $query->where($v, "like", "%" . $value . "%");
            }
    ...
    private function searchOrders_left($value, $key, $flag = 0, $page, $limit)
    {
        ...
        $price_total = \think\Db::name("orders")->alias("o")->leftjoin("clients c", "o.uid=c.id")->leftjoin("currencies cu", "cu.id = c.currency")->leftJoin("user u", "u.id = c.sale_id")->leftJoin("invoices i", "o.invoiceid = i.id")->field("o.id,c.sale_id,o.invoiceid,cu.prefix,cu.suffix,o.amount")->where(function (\think\db\Query $query) use ($param) {
            static $value = NULL;
            foreach ($param as $v) {
                $query->where($v, "like", "%" . $value . "%");
            }
        ...
        foreach ($rows as $k => $row) {
            ...
            } else if ($invoice_type == "zjmf_reinstall_times") {
        ...
        $count = \think\Db::name("orders")->alias("o")->leftjoin("clients c", "o.uid=c.id")->field("c.username,o.id")->where(function (\think\db\Query $query) use ($param) {
            static $value = NULL;
            foreach ($param as $v) {
                $query->where($v, "like", "%" . $value . "%");
            }
    ...
    private function searchInvoice_left($value, $key, $flag = 0, $page, $limit)
    {
        ...
        $rows1 = \think\Db::name("invoices")->alias("i")->field("c.username,i.id,i.uid,i.invoice_num,i.create_time,i.due_time,i.subtotal,cu.prefix,cu.suffix,i.status,i.payment,i.credit,i.type,i.paid_time")->leftjoin("clients c", "c.id=i.uid")->leftJoin("currencies cu", "cu.id = c.currency")->where(function (\think\db\Query $query) use ($param) {
            static $value = NULL;
            foreach ($param as $v) {
                $query->where($v, "like", "%" . $value . "%");
            }
