...
class AppStoreController extends cmf\controller\BaseController
{
    ...
    public function install($id)
    {
        ...
        if ($auth["last_license_time"] + 604800 < time() || ltrim(str_replace("https://", "", str_replace("http://", "", $auth["domain"])), "www.") != ltrim(str_replace("https://", "", str_replace("http://", "", $_SERVER["HTTP_HOST"])), "www.") && php_sapi_name() != "cli" || $auth["installation_path"] != CMF_ROOT || $auth["license"] != configuration("system_license")) {
            return ["status" => 307, "msg" => "授权错误,请检查域名或ip"];
        }
        ...
        if (in_array($res["data"]["product"]["uuid"], $app)) {
            ...
            if ($res["data"]["product"]["app_type"] == "templates") {
                ...
                $content = curl_download($this->market_url . "/download/app_file?id=" . $id . "&from=" . request()->domain() . "/" . adminAddress() . "&token=" . $token . "&time=" . time(), $dir);
                if ($content) {
                    $file = CMF_ROOT . "/public/themes";
