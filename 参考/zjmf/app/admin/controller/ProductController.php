...
class ProductController extends AdminBaseController
{
    ...
    public function saveProductGroup(\think\Request $request)
    {
        if ($request->isPost()) {
            ...
            } else if (!empty($old_customfields)) {
                $des .= "自定义字段移除，";
                $customfields_change = true;
            }
            if (!empty($param["id"])) {
    ...
    public function syncProductInfo()
    {
        ...
        if ($v10ApiId = getZjmfApiIdByProductId($id)) {
            v10ProductInputUpdate($id, $v10ApiId, ["upstream_pid" => $upstream_pid, "description" => $product["description"]]);
            return jsonrule(["status" => 200, "msg" => "同步成功"]);
        }
    ...
    public function edit(\think\Request $request)
    {
        if ($request->isPost()) {
            ...
            array_push($allow_api_type, ["name" => "v10", "name_zh" => "v10"]);
            ...
            if ($api_type != "zjmf_api" && $api_type != "whmcs") {
                ...
                } else if ($product["auto_create_config_options"] == 0 && $param["type"] == "dcimcloud") {
                    if (!empty($param["server_group"])) {
                ...
                } else if ($product["auto_create_config_options"] == 0 && $param["type"] == "cloud" && !empty($param["server_group"])) {
            ...
            if (empty($param["groupid"])) {
                if ($param["type"] == "dcim") {
                    $basedata["groupid"] = 2;
                } else if ($param["type"] == "clound" || $param["type"] == "dcimcloud") {
                    $basedata["groupid"] = 1;
                } else {
                    $basedata["groupid"] = 3;
                }
            ...
            $v10ApidId = getZjmfApiIdByProductId($id);
            if ($v10ApidId) {
                v10ProductInputUpdate($id, $v10ApidId, ["upstream_pid" => $param["upstream_pid"], "description" => $param["description"] ?? "", "upstream_price_value" => $param["upstream_price_value"]]);
            } else if ($api_type == "zjmf_api") {
                ...
                try {
                    ...
                    if ($api_type != "zjmf_api") {
                        ...
                        if ($auto_create_config_options == 1 && $param["type"] == "dcim") {
                            ...
                            } else if ($basedata["config_option1"] == "bms") {
                                $config_group_id = \think\Db::name("product_config_groups")->insertGetId(["name" => $config_group_name, "description" => $config_group_name]);
                                \think\Db::name("product_config_links")->insert(["gid" => $config_group_id, "pid" => $id]);
                    ...
                    if (empty($this->request->afpid)) {
                        ...
                        if ($spg["affiliate_enabled"] != $param["affiliate_enabled"]) {
                            ...
                            if ($spg["affiliate_enabled"] == 1) {
                                $str = "自定义";
                            } else if ($spg["affiliate_enabled"] == 0) {
                                $str = "跟随系统";
                            } else {
                                $str = "关闭";
                            }
                            ...
                            } else if ($param["affiliate_enabled"] == 0) {
    ...
    public function selectType(\think\Request $request)
    {
        ...
        if ($type == "dcim" || $type == "server") {
            $basedata["groupid"] = 1;
        } else if ($type == "clound" || $type == "dcimcloud") {
            $basedata["groupid"] = 2;
        } else {
            $basedata["groupid"] = 3;
        }
