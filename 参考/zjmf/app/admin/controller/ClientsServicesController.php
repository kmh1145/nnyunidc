...
class ClientsServicesController extends GetUserController
{
    public function index(\think\Request $request)
    {
        ...
        } else if ($product["api_type"] == "manual" || $product["api_type"] == "whmcs") {
            $returndata["upstream_host"] = ["amount" => $host_data["upstream_cost"] ?: "-", "amount_desc" => "-", "firstpaymentamount" => "-", "firstpaymentamount_desc" => "-"];
        } else if ($product["api_type"] == "resource" && function_exists("resourceCurl")) {
        ...
        } else if ($product["api_type"] == "resource") {
        ...
        } else if ($product["api_type"] == "manual") {
        ...
        } else if ($product["type"] == "dcimcloud") {
        ...
        if ($product["type"] == "dcim") {
            ...
            } else if ($host_data["dcimid"] == 0) {
                foreach ($admin_auth as $k => $v) {
                    if ($k != "create") {
                        $admin_auth[$k] = "off";
                    }
                }
            } else if ($product_data["api_type"] == "zjmf_api") {
                if (!empty($zjmf_api)) {
    ...
    public function postInfo(\think\Request $request)
    {
        ...
        if ($product_info["api_type"] == "zjmf_api" && $param["dcimid"] != $host_data["dcimid"]) {
            if (getZjmfApiIdByHostId($host_data["id"])) {
                $update_data["dcimid"] = $param["dcimid"];
                $token = md5(randStr(16) . time() . $param["dcimid"]);
                $update_data["stream_info"] = json_encode(["token" => $token]);
                $post_data = ["old_host_id" => $host_data["dcimid"], "product_id" => $product_info["upstream_pid"], "token" => $token, "downstream_host_id" => $host_data["id"], "downstream_url" => trim(request()->domain() . request()->rootUrl(), "/")];
                $res = zjmfCurl($product_info["zjmf_api_id"], "/console/v1/host/" . $param["dcimid"] . "/downstream", $post_data, 30, "POST");
                if ($res["status"] == 200) {
        ...
        } else if ($host_data["nextduedate"] != $param["nextduedate"]) {
        ...
        } else if ($host_data["termination_date"] != $param["termination_date"]) {
        ...
        } else if (in_array($host_data["domainstatus"], ["Cancelled", "Fraud", "Deleted"]) && in_array($param["domainstatus"], ["Pending", "Active", "Suspended"])) {
            $product_info = \think\Db::name("products")->where("id", $host_data["productid"])->find();
