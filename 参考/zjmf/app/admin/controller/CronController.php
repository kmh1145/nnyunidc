    ...
    public function saveCron()
    {
        ...
        foreach ($data as $k => $v) {
            if (isset($params[$k]) && $v != $params[$k]) {
                ...
                } else if ($k == "cron_host_unsuspend_send") {
                    if ($params[$k] == 1) {
                        $dec .= " -  发送解除暂停邮件";
                    } else {
                        $dec .= " -  发送解除不暂停邮件";
                    }
                } else if ($k == " cron_host_suspend_send") {
                    if ($params[$k] == 1) {
                        $dec .= " -  发送暂停邮件";
                    } else {
                        $dec .= " -  发送不暂停邮件";
                    }
                } else if ($k == "cron_host_suspend") {
                    if ($params[$k] == 1) {
                        $dec .= " -  启用暂停功能";
                    } else {
                        $dec .= " -  不启用暂停功能";
                    }
                } else if ($k == "cron_host_suspend_time") {
                    $dec .= " - 暂停时间:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_host_terminate_time") {
                    $dec .= " - 删除时间:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_day_start_time") {
                    $dec .= " - 每天何时:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_default_days") {
                    $dec .= " - 生成账单:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_pay_email") {
                    if ($params[$k] == 1) {
                        $dec .= " -  付款提醒邮件";
                    } else {
                        $dec .= " -  付款提醒邮件";
                ...
                } else if ($k == "cron_invoice_create_hour") {
                    $dec .= " - 小时付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_day") {
                    $dec .= " - 天付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_monthly") {
                    $dec .= " - 月付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_quarterly") {
                    $dec .= " - 季付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_semiannually") {
                    $dec .= " - 半年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_annually") {
                    $dec .= " - 年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_biennially") {
                    $dec .= " - 两年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_triennially") {
                    $dec .= " - 三年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_fourly") {
                    $dec .= " - 四年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_fively") {
                    $dec .= " - 五年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_sixly") {
                    $dec .= " - 六年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_sevenly") {
                    $dec .= " - 七年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_eightly") {
                    $dec .= " - 八年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_ninely") {
                    $dec .= " - 九年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_create_tenly") {
                    $dec .= " - 十年付:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_unpaid_email") {
                    $dec .= " - 账单未付款提醒天数:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_first_overdue_email") {
                    $dec .= " - 第 1 次逾期提醒:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_second_overdue_email") {
                    $dec .= " - 第 2 次逾期提醒:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_third_overdue_email") {
                    $dec .= " - 第 3 次逾期提醒:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_ticket_close_time") {
                    $dec .= " - 关闭工单:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_order_unpaid_time_high") {
                    if ($params[$k] == 1) {
                        $dec .= " -  删除取消功能启用";
                    } else {
                        $dec .= " -  删除取消功能关闭";
                ...
                } else if ($k == "cron_order_unpaid_time") {
                    $dec .= " - 删除取消:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_order_unpaid_action:Delete") {
                    $dec .= " - 操作类型:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_credit_limit_suspend_time") {
                    $dec .= " - 信用额账单未付款到期暂停天数:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_credit_limit_invoice_unpaid_email") {
                    $dec .= " - 信用额账单未付款提醒天数:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_credit_limit_invoice_first_overdue_email") {
                    $dec .= " - 信用额账单第 1 次逾期提醒:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_credit_limit_invoice_second_overdue_email") {
                    $dec .= " - 信用额账单第 2 次逾期提醒:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_credit_limit_invoice_third_overdue_email") {
                    $dec .= " - 信用额账单第 3 次逾期提醒:" . $company_name . "改为" . $params[$k];
                } else if ($k == "cron_invoice_recharge_delete") {
                    if ($params[$k] == 1) {
                ...
                } else if ($k == "cron_invoice_recharge_delete_time") {
