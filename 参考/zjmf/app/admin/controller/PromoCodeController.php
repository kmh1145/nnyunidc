...
class PromoCodeController extends AdminBaseController
{
    ...
    public function add()
    {
        ...
        if (!empty($data["upgrades"])) {
            $upgrades = ["upgrade_type" => "", "upgrade_value" => 0, "upgrade_value_type" => "percent", "upgrade_options" => []];
            if (!empty($params["upgrade_type"]) && in_array($params["upgrade_type"], ["product", "option"])) {
                $upgrades["upgrade_type"] = $params["upgrade_type"];
            }
            if (is_numeric($params["upgrade_value"]) && 0 < $params["upgrade_value"]) {
    ...
    public function savePage()
    {
        ...
        $promo_code["upgrade_value"] = 0;
        $promo_code["upgrade_value_type"] = "percent";
        $promo_code["upgrade_options"] = [];
        if (!empty($promo_code["upgrades"]) && !empty($promo_code["upgrade_config"])) {
    ...
    public function save()
    {
        ...
        if (isset($params["cycles"]) && $params["cycles"] != $code_info["cycles"]) {
    ...
    public function expireImmediately()
    {
        ...
        if (0 < $exist["expiration_time"] && $exist["expiration_time"] < time()) {
            $result["status"] = 200;
