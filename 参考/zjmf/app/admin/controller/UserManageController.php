...
class UserManageController extends GetUserController
{
    ...
    public function summary()
    {
        ...
        $re["being_due_host"] = ["count" => $due_host_total["due_host_count"] ?? 0, "total" => $currency["prefix"] . number_format(round($due_host_total["firstpaymentamount"] ?? 0, 2), 2)];
        ...
        $re["due_host"] = ["count" => $due_total["due_host_count"] ?? 0, "total" => $currency["prefix"] . number_format(round($due_total["firstpaymentamount"] ?? 0, 2), 2)];
        $hostserver = [];
        ...
        foreach ($list as $k => $plugin) {
            if (!$plugin["config"]) {
            } else if (!empty($clients_oauth[$plugin["name"]])) {
    ...
    public function hostByUid()
    {
        ...
        if ($params["source"] == "ticket") {
            ...
            foreach ($host_list as &$v) {
                ...
                $v["billingcycle"] = config("billing_cycle")[$v["billingcycle"]] ?? $v["billingcycle"];
            ...
            foreach ($hosts as $hkey => $host) {
                $host["billingcycle"] = config("billing_cycle")[$host["billingcycle"]] ?? $host["billingcycle"];
    ...
    public function profilePost()
    {
        if ($this->request->isPost()) {
            ...
            if ($clients["currency"] != $params["currency"] && 0 < $clients["credit"]) {
    ...
    {
        $param = $this->request->param();
        $page = isset($param["page"][0]) && 1 <= $param["page"] ? (int) $param["page"] : 1;
        $limit = isset($param["limit"][0]) && 10 <= $param["limit"] ? (int) $param["limit"] : 10;
        $order = isset($param["order"][0]) ? (string) $param["order"] : "id";
    ...
    public function cerifyLogList22()
    {
        ...
        $sort = isset($param["sort"][0]) && $param["sort"] === "ASC" ? "ASC" : "DESC";
        $where = [];
        if (isset($param["type"]) && 0 < $param["type"]) {
            $where[] = ["a.type", "=", $param["type"]];
        }
        if (isset($param["status"]) && 0 < $param["status"]) {
        ...
        $keywords_where = function (\think\db\Query $query) {
        ...
        foreach ($list as &$v) {
            ...
            if ($v["custom_fields_log"]) {
                ...
                foreach ($customfields as $customfield) {
                    $i++;
                    $i <= 10 or $v["custom_fields_log_arr"][] = ["type" => $customfield["type"], "title" => $customfield["title"], "val" => $v["custom_fields_log"]["custom_fields" . $i] ?? ""];
                    $i <= 10 && $v["custom_fields_log_arr"];
                }
            }
        }
        return jsonrule(["status" => 200, "msg" => lang("SUCCESS MESSAGE"), "data" => ["list" => $list, "total" => $count, "page" => $page, "limit" => $limit]]);
    ...
    public function certifiPersonModify()
    {
        if ($this->request->isPost()) {
            ...
            if ($result["status"] == 200) {
                ...
                list($certifi["img_one"], $certifi["img_two"]) = $addresses;
                $certifi["status"] = 3;
    ...
    public function certifiCompanyModify()
    {
        if ($this->request->isPost()) {
            ...
            if ($result["status"] == 200) {
                $addresses = explode(",", $result["savename"]);
                list($certifi["img_one"], $certifi["img_two"], $certifi["img_three"]) = $addresses;
                $certifi["status"] = 3;
    ...
    public function certifiStatus()
    {
        ...
        if (!isset($params["status"]) || 5 <= $params["status"] || $params["status"] <= 0) {
    ...
    public function get_search($param)
    {
        ...
        if (!empty($param["payment"])) {
            ...
            } else if ($param["payment"] == "creditPay") {
                $where[] = ["i.credit", ">", 0];
            } else {
        ...
        if (isset($param["create_time"]) && 0 < $param["create_time"]) {
        ...
        try {
            ...
            if ($log) {
                ...
                if ($log["custom_fields_log"]) {
                    ...
                    foreach ($customfields as $customfield) {
                        $i++;
                        $i <= 10 or $log["custom_fields_log_arr"][$customfield["field"]] = ["type" => $customfield["type"], "title" => $customfield["title"], "val" => $log["custom_fields_log"]["custom_fields" . $i] ?? ""];
                        $i <= 10 && $log["custom_fields_log_arr"][$customfield["field"]];
                    }
                }
            }
