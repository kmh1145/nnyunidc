    ...
    public function saleRecords()
    {
        ...
        foreach ($list as $key => $val) {
            ...
            if (!empty($ladder["turnover"]["turnover"])) {
                ...
                } else if ($val["updategrade"] == 0 && $val["type"] == "upgrade" || $val["updategrade"] == 0 && $val["typess"] == "upgrade" && $val["type"] == "discount") {
                    $list[$key]["bates"] = 0;
                    $list[$key]["batesamount"] = "0.00+" . bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 2);
                    if (bcmul($ladder["turnover"]["bates"] / 100, $val["amount"], 2) == 0) {
                        $list[$key]["batesamount"] = "0.00";
                    }
                } else if ($val["is_renew"] == 1 && $val["type"] == "renew" || $val["is_renew"] == 1 && $val["typess"] == "renew" && $val["type"] == "discount") {
                ...
                } else if ($val["updategrade"] == 1 && $val["type"] == "upgrade" || $val["updategrade"] == 1 && $val["typess"] == "upgrade" && $val["type"] == "discount") {
            ...
            } else if ($val["is_renew"] == 0 && $val["type"] == "renew" || $val["is_renew"] == 0 && $val["typess"] == "renew" && $val["type"] == "discount") {
                $list[$key]["batesamount"] = "0.00";
                $list[$key]["bates"] = 0;
            } else if ($val["updategrade"] == 0 && $val["type"] == "upgrade" || $val["updategrade"] == 0 && $val["typess"] == "upgrade" && $val["type"] == "discount") {
                $list[$key]["batesamount"] = "0.00";
        ...
        foreach ($arr1 as $key => $val) {
            ...
            } else if ($val["type"] != "续费" && $val["type"] != "升级") {
        ...
        foreach ($arr1 as $key => $val) {
            ...
            foreach ($val["child"] as $keys => $vals) {
                ...
                } else if ($vals["updategrade"] == 1 && $vals["type"] == "升级") {
    ...
    private function getLastLadder($this_month_sale_total = 0, $now_or_last = "last")
    {
        ...
        if ($sale_ladder) {
            foreach ($sale_ladder as $key => $item) {
                ...
                } else if ($now_or_last == "now" && $item["turnover"] <= $this_month_sale_total) {
                    $ladder["turnover"] = $item["turnover"];
                    $ladder["bates"] = $item["bates"];
                }
            }
