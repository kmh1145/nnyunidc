...
class AffiliateController extends GetUserController
{
    public function index(\think\Request $request)
    {
        ...
        $data = \think\Db::name("affiliates")->alias("a")->field("a.*,c.username,c.companyname")->join("clients c", "a.uid=c.id")->where($where)->where("c.username LIKE '%" . $username . "%'")->where(function (\think\db\Query $query) use ($params) {
            if (!empty($params["visitors"]) && !empty($params["visitors_type"])) {
            ...
            if (!empty($params["balance"]) && !empty($params["balance_type"])) {
            ...
            if (!empty($params["withdrawn"]) && !empty($params["withdrawn_type"])) {
            ...
            if (!empty($params["registcount"]) && !empty($params["registcount_type"])) {
    ...
    public function useraffibuyrecord(\think\Request $request)
    {
        ...
        foreach ($rows as $k => $value) {
            ...
            if ($value["type"] == "renew") {
                ...
                $rows[$k]["type"] = "新订购";
            } else if ($value["type"] == "product2") {
                $rows[$k]["type"] = "二次订购";
            } else if ($value["type"] == "upgrade") {
                $rows[$k]["type"] = "升降级";
            } else if ($value["type"] == "zjmf_flow_packet") {
                $rows[$k]["type"] = "流量包订购";
            } else if ($value["type"] == "zjmf_reinstall_times") {
                $rows[$k]["type"] = "重装次数";
            }
        }
        foreach ($rows as $k => $value) {
