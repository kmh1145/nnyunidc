...
class ZjmfFinanceApiController extends GetUserController
{
    ...
    {
        $params = input("post.");
        $type = ["zjmf_api" => "智简魔方", "manual" => "手动", "v10" => "v10"];
        if ($params["type"] == "manual") {
            ...
            $data = ["id" => $id];
        } else if ($params["type"] == "whmcs" || $params["type"] == "v10") {
            $validate = new \app\admin\validate\ZjmfFinanceApiValidate();
            $validate_result = $validate->check($params);
            if (!$validate_result) {
                return jsonrule(["status" => 406, "msg" => $validate->getError()]);
            ...
            $id = \think\Db::name("zjmf_finance_api")->insertGetId($insert);
            if ($params["type"] == "v10") {
                $res = zjmfCurl($id, "/api/v1/product", [], 15, "GET");
                if ($res["status"] == 200) {
                    \think\Db::name("zjmf_finance_api")->where("id", $id)->update(["product_num" => count($res["data"]["list"] ?? [])]);
                }
            }
        }
    ...
    public function index()
    {
        ...
        $data = \think\Db::name("zjmf_finance_api")->alias("a")->field("a.id,a.type,a.type as type_zh,a.contact_way,a.name,a.des,a.hostname,a.username,a.password,a.status,a.product_num,count(DISTINCT b.id) set_product_num,count(DISTINCT c.id) active_host_num,count(DISTINCT d.id) host_num")->leftJoin("products b", "a.id=b.zjmf_api_id OR a.id=b.upper_reaches_id")->leftJoin("host c", "b.id=c.productid AND domainstatus=\"Active\"")->leftJoin("host d", "b.id=d.productid")->whereLike("a.hostname|a.username", "%" . $search . "%")->where($where)->withAttr("type_zh", function ($value) {
            $type = ["zjmf_api" => "智简魔方", "manual" => "手动", "whmcs" => "WHMCS", "v10" => "v10"];
            return $type[$value];
    ...
    public function apiProducts()
    {
        ...
        foreach ($products as &$v) {
            ...
            if ($v["upstream_cycle"]) {
                ...
                $v["setup_fee"] = 0;
                $v["product_count"] = $v["upstream_price"];
            }
    ...
    public function apiOrder()
    {
        ...
        $fun = function (\think\db\Query $query) use ($params) {
            ...
            if (isset($params["payment"]) && !empty($params["payment"])) {
                ...
                } else if ($params["payment"] == "creditPay") {
    ...
    public function downstreamSummary()
    {
        ...
        if ($api["type"] == "v10") {
            $client = [];
            $client["host_count"] = \think\Db::name("host")->alias("a")->leftJoin("products b", "a.productid=b.id")->where("b.zjmf_api_id", $id)->count();
            $client["active_count"] = \think\Db::name("host")->alias("a")->leftJoin("products b", "a.productid=b.id")->where("b.zjmf_api_id", $id)->where("a.domainstatus", "Active")->count();
            $client["agent_count"] = \think\Db::name("products")->where("zjmf_api_id", $id)->count();
            $data = ["client" => $client];
            return jsonrule(["status" => 200, "msg" => lang("SUCCESS MESSAGE"), "data" => $data]);
        }
    ...
    public function addPage()
    {
        ...
        $apis = \think\Db::name("zjmf_finance_api")->field("id,name,type")->select()->toArray();
        $re["status"] = 200;
    ...
    public function inputProduct(\think\Request $request)
    {
        ...
        $api = \think\Db::name("zjmf_finance_api")->where("id", $param["zjmf_finance_api_id"])->find();
        \think\Db::startTrans();
        try {
            foreach ($productnames as $k => $v) {
                ...
                if ($api["type"] == "v10") {
                    if (isset($type[$k])) {
                        if ($type[$k] == "mf_dcim") {
                            $productType = "dcim";
                        } else if ($type[$k] == "mf_cloud") {
                            $productType = "dcimcloud";
                        }
                    }
                    $pid = $res["id"] ?? 0;
                    $data = ["type" => $productType ?? "other", "upstream_product_shopping_url" => rtrim($api["hostname"], "/") . "/cart/goods.htm?id=" . $k . "&name=" . $v, "api_type" => "zjmf_api", "upstream_pid" => $k, "zjmf_api_id" => $param["zjmf_finance_api_id"], "upstream_price_value" => $param["upstream_price_value"], "server_group" => $param["zjmf_finance_api_id"]];
                    v10ProductInputUpdate($pid, $param["zjmf_finance_api_id"], $data);
                } else {
    ...
    public function upstreamCredit()
    {
        ...
        $api = \think\Db::name("zjmf_finance_api")->where("id", $id)->find();
        if ($api["type"] == "v10") {
            $res = zjmfCurl($id, "console/v1/account", [], 30, "GET");
            if ($res["status"] == 200) {
                $res["data"]["account"]["currency"]["prefix"] = $res["data"]["account"]["currency_prefix"] ?? "￥";
                $result["data"] = $res["data"]["account"];
                $result["status"] = 200;
            } else {
                $result["status"] = 400;
            }
