...
class ConfigOptionsController extends AdminBaseController
{
    public function groupsList()
    {
        ...
        $list = \think\Db::name("product_config_groups")->where("global", 1)->where(function (\think\db\Query $query) {
        ...
        foreach ($list as $key => $value) {
            $products = \think\Db::name("product_config_links")->alias("a")->field("b.id,b.name,b.type")->leftJoin("products b", "a.pid = b.id")->where("a.gid", $value["id"])->where(function (\think\db\Query $query) {
                static $value = NULL;
            })->select()->toArray();
    ...
    public function editGroups()
    {
        $params = $this->request->param();
        $where = [];
        if (isset($params["type"]) && 0 < $params["type"] && $params["type"] < 4) {
            $where[] = ["pg.type", "=", (int) $params["type"]];
        }
    ...
    public function editGroupsPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($product["api_type"] == "zjmf_api" && 0 < $pcg["upstream_id"]) {
            ...
            if ($paytype == "onetime") {
                ...
                foreach ($currencies as $v) {
                    ...
                    foreach ($price_type as $vv) {
                        if (0 <= $product_pricing[$vv[0]] && $vv[0] != "onetime" && $vv[0] != "free") {
                            $field[] = $vv[0];
                            $field[] = $vv[1];
    ...
    public function addOptions()
    {
        if ($this->request->isPost()) {
            ...
            if (!getEdition() && 1 < $param["qty_stage"]) {
    ...
    public function configOptionsOs()
    {
        ...
        } else if ($server_info["server_type"] == "dcimcloud") {
    ...
    public function editConfigPost()
    {
        if ($this->request->isPost()) {
            ...
            try {
                ...
                if (0 < $pco["upstream_id"] && $product["upstream_price_type"] == "percent") {
                } else if (isset($param["price"]) && is_array($param["price"])) {
                    $pricing = $param["price"];
