...
class TicketController extends GetUserController
{
    ...
    public function getList()
    {
        ...
        } else if (!isset($params["status"]) || $params["status"] == "pending") {
        ...
        foreach ($data as $k => $v) {
            ...
            } else if ($v["status"] == 2 && $v["is_deliver"] == 1) {
                $ticket_reply = \think\Db::name("ticket_reply")->where("tid", $v["id"])->order("id", "desc")->find();
    ...
    public function ticketDetail($id = 0)
    {
        ...
        $list[] = ["id" => $id, "uid" => $ticket["uid"], "admin_id" => (int) $ticket["admin_id"], "type" => "t", "content" => $ticket["content"], "attachment" => isset($ticket["attachment"][0]) ? array_map(function ($v) use ($url) {
            return $url . $v;
        ...
        foreach ($reply as $v) {
            ...
            if ($v["is_receive"] == 1 && $v["source"] == 1) {
                ...
                $v["user"] = "客服";
            } else if ($v["is_receive"] == 1 && $v["source"] == 0) {
                $v["deliver_status"] = 1;
        ...
        } else if ($ticket["status"] == 2 && $ticket["is_deliver"] == 1) {
            $ticket_reply = \think\Db::name("ticket_reply")->where("tid", $ticket["id"])->order("id", "desc")->find();
            if ($ticket_reply["is_receive"] == 1 && $ticket_reply["source"] == 1) {
