        ...
        try {
            ...
            foreach ($param as $k => $v) {
                ...
                if ($v != $tmp["value"]) {
                    ...
                    } else if ($k == "certifi_alipay_biz_code") {
                        $tmp["setting"] = "认证方式";
                        $tmp["value"] = $arr1[$tmp["value"]];
                        $v = $arr1[$v];
                        $dec .= $tmp["setting"] . "由“" . $tmp["value"] . "”改为“" . $v . "”，";
                    } else if ($k == "certifi_app_id") {
                        $tmp["setting"] = "APP ID";
                        $dec .= $tmp["setting"] . "由“" . $tmp["value"] . "”改为“" . $v . "”，";
                    } else if ($k == "certifi_alipay_public_key") {
                        $tmp["setting"] = "支付宝公钥";
                        $dec .= $tmp["setting"] . "有修改，";
                    } else if ($k == " certifi_merchant_private_key") {
                        $tmp["setting"] = "商户私钥";
                        $dec .= $tmp["setting"] . "有修改，";
                    } else if ($k == " certifi_appcode") {
                        $tmp["setting"] = "三要素code";
                        $dec .= $tmp["setting"] . "由“" . $tmp["value"] . "”改为“" . $v . "”，";
                    } else if ($k == " certifi_three_type") {
                        $tmp["setting"] = "三要素类型";
                        $dec .= $arr2[$tmp["setting"]] . "由“" . $tmp["value"] . "”改为“" . $arr2[$v] . "”，";
                    } else if ($k == " certifi_realname") {
                        $tmp["setting"] = "同步名字";
                        $dec .= $arr2[$tmp["setting"]] . "由“" . $tmp["value"] . "”改为“" . $arr2[$v] . "”，";
                    } else if ($k == "certifi_is_stop") {
                        $tmp["setting"] = "未实名暂停产品";
                        if ($tmp["value"] == 1) {
                            $dec .= $tmp["setting"] . "由“开启”改为“关闭”，";
                        } else {
                    ...
                    } else if ($k == "certifi_stop_day") {
                        $tmp["setting"] = "暂停期限";
                        $dec .= $tmp["setting"] . "由“" . $tmp["value"] . "”改为“" . $v . "”，";
                    } else if ($k == "certifi_isbindphone") {
                        $tmp["setting"] = "绑定手机是否一致";
                        $dec .= $tmp["setting"] . "由“" . $tmp["value"] . "”改为“" . $v . "”，";
                    } else if ($k == "certifi_isrealname") {
                        $tmp["setting"] = "是否实名";
                        $dec .= $tmp["setting"] . "由“" . $tmp["value"] . "”改为“" . $v . "”，";
                    } else {
                        $dec .= $tmp["setting"] . "由“" . $tmp["value"] . "”改为“" . $v . "”，";
