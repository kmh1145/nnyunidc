...
class HostController extends cmf\controller\HomeBaseController
{
    ...
    public function module()
    {
        ...
        if ($host_data["api_type"] == "zjmf_api") {
            ...
            if (0 < $host_data["bwlimit"]) {
                $flowpacket = parent::name("dcim_flow_packet")->field("id,name,capacity,price,sale_times,stock")->where("status", 1)->whereRaw("FIND_IN_SET('" . $host_data["productid"] . "', allow_products)")->select()->toArray();
                if (!empty($flowpacket)) {
    ...
    public function status()
    {
        ...
        if ($type == "reinstall") {
            ...
            } else if ($result["data"]["last_result"]) {
    ...
    public function custom()
    {
        ...
        $key = config("jwtkey");
        list($jwt) = explode(" ", $header["authorization"]);
        try {
