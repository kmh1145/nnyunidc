...
class CartController extends cmf\controller\HomeBaseController
{
    public function products()
    {
        ...
        } else if (isset($param["group_id"])) {
            $group_id = intval($param["group_id"]);
            $fgid = \think\Db::name("product_groups")->where("id", $group_id)->value("gid") ?: 0;
        } else if (isset($param["first_group_id"])) {
        ...
        $fgs = \think\Db::name("product_first_groups")->field("id,name")->where("hidden", 0)->where(function (\think\db\Query $query) {
            static $fgid = NULL;
            if (isset($fgid)) {
                $query->where("id", $fgid);
            }
            ...
            if (!empty($product_error)) {
                ...
                if ($pay_type["pay_type"] == "free") {
                    ...
                    if (!empty($promo) && 0 < $product_total_price_sale) {
                        ...
                        } else if ((empty($promo["appliesto"][0]) || in_array($v["pid"], $promo["appliesto"])) && (empty($promo["cycles"]) || in_array($v["billingcycle"], explode(",", $promo["cycles"])))) {
