...
class InvoicesController extends app\home\controller\CommonController
{
    ...
    public function accountsRecord()
    {
        ...
        $accounts = \think\Db::name("accounts")->alias("a")->leftJoin("invoices b", "a.invoice_id = b.id")->field("a.id,a.invoice_id,a.pay_time,a.gateway as payment_zh,a.description,b.type,a.amount_in,a.trans_id")->where($where)->withAttr("payment_zh", function ($value, $data) {
            static $gateways = NULL;
            foreach ($gateways as $v) {
                if ($v["name"] == $value) {
                    return $v["title"];
