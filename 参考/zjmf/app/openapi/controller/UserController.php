...
class UserController extends cmf\controller\HomeBaseController
{
    ...
    public function initialize()
    {
        ...
        if (in_array($action, $_action) && request()->id) {
            ...
            if ($client["second_verify"] == 1 && !empty($second_verify_home) && !empty($second_verify_action) && empty($verification_success_1) && empty($verification_success_2)) {
                ...
                if (!empty($is_certifi) && $is_certifi[$action] == 1 && !checkCertify(request()->uid)) {
    ...
    public function loginNotice()
    {
        ...
        if ($type == "email") {
            ...
            if ($status === 0) {
                ...
                $tmp = (int) cache("verification_code_login_notice_email" . $user["email"]);
                if ($code !== $tmp) {
                    return json(["status" => 400, "msg" => "Incorrect verification code"]);
                }
            }
        ...
        if ($status === 0) {
            ...
            $tmp = (int) cache("verification_code_login_notice_phone" . $user["phone_code"] . $user["phonenumber"]);
            if ($code !== $tmp) {
                return json(["status" => 400, "msg" => "Incorrect verification code"]);
            }
        }
        ...
        if ($certifi_type != "artificial") {
            ...
            if (!empty($customfields[0])) {
                ...
                foreach ($customfields as $customfield) {
                    ...
                    } else if ($customfield["type"] == "select" && !in_array($data[$customfield["field"]], array_keys($customfield["options"]))) {
    ...
    public function companyRealNameAuth()
    {
        if ($this->request->isPost()) {
            ...
            if ($cardtype == 0) {
                ...
                if (isset($res[0])) {
                    list($certifi["img_one"], $certifi["img_two"]) = $res;
                }
            ...
            if ($cardtype == 1) {
                ...
                $certifi["img_four"] = $img_four;
                if (isset($upload[0])) {
                    list($certifi["img_one"], $certifi["img_two"]) = $upload;
                }
                $pic = implode(",", [$certifi["img_one"], $certifi["img_two"], $certifi["img_three"], $certifi["img_four"]]);
