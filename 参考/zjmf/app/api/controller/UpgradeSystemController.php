...
class UpgradeSystemController extends think\Controller
{
    public function sysVersion()
    {
        ...
        while (($filename = readdir($handler)) !== false) {
            ...
            } else if (preg_match("/" . $last_version . "\\.zip\$/i", $filename)) {
                $is_download = 1;
            }
        }
        closedir($handler);
    ...
    public function getAutoUpdate()
    {
        ...
        if (empty($zjmf_authorize)) {
            return jsonrule(["status" => 400, "msg" => "授权错误,请检查域名或ip"]);
        }
        ...
        if ($ip != $auth["ip"] && !empty($ip)) {
            return jsonrule(["status" => 400, "msg" => "授权错误,请检查ip"]);
        }
        ...
        if (!empty($auth["facetoken"])) {
            return jsonrule(["status" => 400, "msg" => "您的授权已被暂停,请前往智简魔方会员中心检查授权状态"]);
        }
        if ($auth["status"] == "Suspend") {
            return jsonrule(["status" => 400, "msg" => "您的授权已被暂停,请前往智简魔方会员中心检查授权状态"]);
        }
        if (!empty($auth["due_time"]) && $auth["due_time"] < time() && $auth["edition"] == 1) {
            return jsonrule(["status" => 400, "msg" => "您的升级与支持服务已到期，无法升级"]);
        }
        $handler = opendir(CMF_ROOT);
        while (($filename = readdir($handler)) !== false) {
            if ($filename == "." && $filename == "..") {
    ...
    public function getCheckAutoUpdate()
    {
        ...
        if ($data["progress"] == "20%" && $data["status"] == 200) {
            ...
            $data["progress_msg"] = bcmul(0 + 0 * bcdiv($moment_size, $origin_size, 2), 100, 2) . "%";
    ...
    public function getSqlUpdate()
    {
        ...
        if (version_compare($last_version, $version, ">")) {
            foreach ($arr as $v) {
                ...
                if (version_compare($sql_version, $version, ">") && file_exists($sql_file)) {
