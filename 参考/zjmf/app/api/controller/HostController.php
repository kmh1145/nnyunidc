...
class HostController
{
    ...
    public function syncInfo()
    {
        ...
        $params["token"] = $token;
        if (getZjmfApiIdByHostId($id)) {
            $validate = ["id" => $id, "token" => $params["token"], "rand_str" => $params["rand_str"]];
            if (!validateSign($validate, $params["signature"])) {
                $result["status"] = 400;
                $result["msg"] = "签名验证失败";
                return json($result);
            }
        } else if (!validateSign($params, $params["signature"])) {
            $result["status"] = 400;
            $result["msg"] = "签名验证失败";
            return json($result);
        }
        $update = ["domain" => $params["domain"], "username" => $params["username"], "password" => cmf_encrypt($params["password"]), "os" => $params["os"], "os_url" => $params["os_url"] ?: "", "dedicatedip" => $params["dedicatedip"], "assignedips" => $params["assignedips"], "port" => (int) $params["port"], "suspendreason" => $params["suspendreason"]];
